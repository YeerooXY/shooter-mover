...
const defaultCatalog = [
 {id:"enemy.moving-droid",label:"Moving Droid (Level 1 legacy)",type:"enemy",source:"built-in Level1"},
...
function initialState(){
 ...
 return {
  ...
  level:{id:"level.level-1",name:"Level 1",targetFolder:"Level1",startRoomId:r.id,finalRoomId:r.id,finalExitDoorId:exit.id,includeBridge:true},
...
function renderHeaderFields(){
 syncWorkspaceMode();
 $("#levelId").value=state.level.id;$("#levelName").value=state.level.name;$("#targetFolder").value=state.level.targetFolder;$("#includeBridge").checked=state.level.includeBridge!==false;
...
function renderAssets(){
 ...
 </div>`).join("")||`<div class="notice">No matching assets. Scan the Unity project or add an ID manually.</div>`;
