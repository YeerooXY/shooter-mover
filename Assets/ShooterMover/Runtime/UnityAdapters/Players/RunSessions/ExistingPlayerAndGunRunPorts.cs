...
{
    /// <summary>
    /// Narrow run-session adapter over the already-constructed PlayerSetup.
    /// It does not create or own another PlayerActorState.
    /// </summary>
    public sealed class ExistingPlayerLiveRunPort :
        IRunPlayerLivePort
    {
        private readonly PlayerSetup runtime;
        ...
        {
            PlayerSnapshot snapshot = runtime.ExportSnapshot();
            ...
            long authoritativeTick)
        {
            ...
            PlayerSnapshot before = runtime.ExportSnapshot();
            ...
            bool succeeded = result != null
                && (result.Status == PlayerRestartStatus.Applied
                    || result.Status == PlayerRestartStatus.Duplicate);
            return new RunLivePortRestartResult(
                succeeded,
                result == null
    ...
    /// <summary>
    /// Run lifecycle projection over existing inventory-backed gun execution. Cooldown
    /// and fire replay remain in GunExecutionCore and are generation-keyed. This port
    /// only freezes exact equipment identities and clears authored transient effects.
    /// </summary>
    public sealed class ExistingGunExecutionRunPort :
        IRunGunLivePort
    {
        ...
        public ExistingGunExecutionRunPort(
            ...
            Action clearContactOperations)
        {
                ...
                || ids.Distinct().Count() != ids.Count)
            {
                throw new ArgumentException(
                    "Frozen gun equipment identities must be non-null and unique.",
                    nameof(frozenEquipmentInstanceStableIds));
            }
            ids.Sort();
        ...
        public string PortId
        {
            get { return "inventory-gun-execution"; }
        }
            ...
            long authoritativeTick)
        {
            if (retiringLifecycleGeneration != lifecycleGeneration)
            {
                return retiringLifecycleGeneration < lifecycleGeneration
                    ? "gun-runtime-stale-generation"
                    : "gun-runtime-future-generation";
            }
            return replacementLifecycleGeneration
                == retiringLifecycleGeneration + 1L
                ? string.Empty
                : "gun-runtime-generation-invalid";
        }
