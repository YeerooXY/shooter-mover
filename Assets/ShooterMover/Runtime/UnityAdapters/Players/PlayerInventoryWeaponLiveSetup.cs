...
{
    public sealed class PlayerLiveWeaponStateBridge :
        IInventoryWeaponActorStateSource,
        IWeaponActorOwnershipResolver
    {
        private readonly PlayerLiveSetup playerRuntime;

        public PlayerLiveWeaponStateBridge(PlayerLiveSetup runtime)
        {
    ...
    /// </summary>
    public sealed class PlayerInventoryWeaponLiveSetupRoot : IDisposable
    {
        private PlayerInventoryWeaponLiveSetupRoot(
            PlayerLiveWeaponStateBridge playerState,
            RouteProfileActiveWeaponSource activeWeapon,
            InventoryWeaponLiveSetup runtime)
        {
        ...
        public PlayerLiveWeaponStateBridge PlayerState { get; }
        public RouteProfileActiveWeaponSource ActiveWeapon { get; }
        public InventoryWeaponLiveSetup Runtime { get; }

        public static PlayerInventoryWeaponLiveSetupRoot CreateCanonical(
            PlayerLiveSetup playerRuntime,
            ...
            IWeaponAugmentModifierSetResolver augmentModifierResolver)
        {
            ...
            int activeSlotIndex = FindExactRouteSlot(
                route,
                exact.InstanceId);
            var playerState = new PlayerLiveWeaponStateBridge(
                playerRuntime ?? throw new ArgumentNullException(nameof(playerRuntime)));
            var activeWeapon = new RouteProfileActiveWeaponSource(
                route,
                activeSlotIndex);
            var executionAdapter = new InventoryBackedWeaponExecutionBridge(
                new WeaponEquipmentViewLookup(
            ...
            var runtime = new InventoryWeaponLiveSetup(
                ...
                activeWeapon,
                executionAdapter);
            return new PlayerInventoryWeaponLiveSetupRoot(
                playerState,
                activeWeapon,
                runtime);
        }

        public static PlayerInventoryWeaponLiveSetupRoot Create(
            PlayerLiveSetup playerRuntime,
