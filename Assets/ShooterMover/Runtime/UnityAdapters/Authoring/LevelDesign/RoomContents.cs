...
namespace ShooterMover.UnityAdapters.Authoring.LevelDesign
{
    ...
    /// </summary>
    [CreateAssetMenu(
        fileName = "RoomContents",
        menuName = "Shooter Mover/Level Design/Room Content Definition 2D")]
    public sealed class RoomContents : ScriptableObject
    {
        ...
        [SerializeField] private RoomObject[] placements =
            Array.Empty<RoomObject>();
        ...
        public RoomObject[] Placements =>
            placements == null
                ? Array.Empty<RoomObject>()
                : (RoomObject[])placements.Clone();
        ...
        public bool TryValidate(out string error)
        {
            ...
            RoomObject[] authoredPlacements = placements
                ?? Array.Empty<RoomObject>();
            for (int index = 0; index < authoredPlacements.Length; index++)
            {
                RoomObject placement = authoredPlacements[index];
                if (placement == null)
                {
        ...
        public void ConfigureForTests(
            ...
            RoomObject[] configuredPlacements)
        {
            ...
            placements = configuredPlacements == null
                ? Array.Empty<RoomObject>()
                : (RoomObject[])configuredPlacements.Clone();
        }
    }
}
