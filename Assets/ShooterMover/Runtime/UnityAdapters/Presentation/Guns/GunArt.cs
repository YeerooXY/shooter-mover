...
namespace ShooterMover.UnityAdapters.Presentation.Guns
{
    public sealed class GunArtSpriteResolution
    {
        public GunArtSpriteResolution(
    ...
    public static class GunArt
    {
        private static readonly Dictionary<string, string> ResourceByArtId =
            ...
            {
                { "gun-art.blaster.side-v1", "GunArt/blaster_sp" },
                { "gun-art.shotgun-basic.side-v1", "GunArt/shotgun_basic_sp" },
                { "gun-art.rocket-launcher.side-v1", "GunArt/rocket_launcher_sp" },
                { "gun-art.arc-rifle.side-v1", "GunArt/arc_rifle_sp" },
                { "gun-art.ricochet-gun.side-v1", "GunArt/ricochet_gun_sp" },
            };

        private static readonly Dictionary<string, GunArtSpriteResolution>
            Cache = new Dictionary<string, GunArtSpriteResolution>(
        ...
        public static GunArtSpriteResolution Preload(
            string artReferenceId)
        {
            string canonical = string.IsNullOrWhiteSpace(artReferenceId)
                ? string.Empty
                : artReferenceId.Trim();
            GunArtSpriteResolution cached;
            ...
            string diagnostic = fallback
                ? "gun-art-sprite-missing:" + canonical
                : string.Empty;
            if (fallback)
            {
                ...
                {
                    Debug.LogWarning(
                        "Gun side-profile art could not be resolved for exact art ID '"
                        + canonical + "'. The generic silhouette will be used.");
                }
            }

            var resolution = new GunArtSpriteResolution(
                canonical,
                sprite,
                fallback,
                diagnostic);
        ...
        private static Sprite GetFallbackSprite()
        {
            ...
            fallbackSprite = Sprite.Create(
                ...
                100f);
            fallbackSprite.name = "GunArtFallbackSprite";
