...
namespace ShooterMover.UnityAdapters.Rewards.Pickups
{
    ...
    public sealed class LootPickup : MonoBehaviour, IRestartParticipant
    {
        ...
        [SerializeField] private LootPickupPresentationStyle[] presentationStyles =
        ...
        private LootPickupPayload payload;
        private bool collectInProgress;
        private bool collected;
        private LootPickupCollectResult lastCollectResult;
        private RestartParticipantRegistrationResult lastRestartRegistration;

        public LootPickupPayload Payload { get { return payload; } }
        public bool IsCollected { get { return collected; } }
        public float CollectionRadius { get { return collectionRadius; } }
        public LootPickupCollectResult LastCollectResult { get { return lastCollectResult; } }
        ...
        public void ConfigureForTests(
            ...
            IEnumerable<LootPickupPresentationStyle> styles,
            bool registerForRestart = true)
        {
            ...
            presentationStyles = styles == null
                ? new LootPickupPresentationStyle[0]
                : new List<LootPickupPresentationStyle>(styles).ToArray();
            this.registerForRestart = registerForRestart;
            Configure(payload, lifecycleAuthority, restartScope);
        }

        public LootPickupCollectResult TryCollect(StableId claimantStableId)
        {
            if (claimantStableId == null)
            {
                lastCollectResult = new LootPickupCollectResult(
                    LootPickupCollectStatus.Invalid,
                    null,
            ...
            {
                lastCollectResult = new LootPickupCollectResult(
                    LootPickupCollectStatus.AlreadyCollectedNoChange,
            ...
            {
                return new LootPickupCollectResult(
                    LootPickupCollectStatus.PendingRetry,
                    lastCollectResult == null ? null : lastCollectResult.AuthorityResult,
            ...
            try
            {
                lastCollectResult = authority.Collect(payload, claimantStableId)
                    ?? new LootPickupCollectResult(
                        LootPickupCollectStatus.Rejected,
                        null,
        ...
        public void HandleTriggerForTests(LootCollector claimant)
        {
            StableId claimantId;
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            ...
            LootCollector claimant = other.GetComponentInParent<LootCollector>();
        ...
        private void ApplyConfiguredPresentation()
        {
            ...
            {
                LootPickupPresentationStyle style = presentationStyles[index];
