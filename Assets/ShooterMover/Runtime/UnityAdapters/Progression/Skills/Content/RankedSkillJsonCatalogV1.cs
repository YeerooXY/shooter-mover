...
namespace ShooterMover.UnityAdapters.Progression.Skills.Content
{
    ...
    public sealed class CombinedSkillRankRequirementV1
    {
        ...
        {
            var ids = (skillIds ?? throw new ArgumentNullException(nameof(skillIds)))
                .Select(x => (x ?? string.Empty).Trim()).ToList();
    ...
    {
        public ImportedSkillSynergyV1(
            SkillSynergyDefinitionV2 definition,
            string displayName,
            string description,
            IEnumerable<string> eligibleClassIds,
            IEnumerable<CombinedSkillRankRequirementV1> combinedRequirements)
        {
        ...
        public bool IsSatisfied(RankedSkillAllocationSnapshotV2 allocation)
        {
            ...
            return Definition.Requirements.All(x => allocation.RankOf(x.SkillId) >= x.MinimumRank)
                && CombinedRequirements.All(x => x.IsSatisfied(allocation));
        }
    }
    ...
    public sealed class RankedSkillCatalogSummaryV1
    {
        public RankedSkillCatalogSummaryV1(RankedSkillCatalogV2 catalog, IEnumerable<string> classIds)
        {
            ...
            SkillsByCategory = new ReadOnlyDictionary<string, int>(catalog.Skills
                .GroupBy(x => x.CategoryId, StringComparer.Ordinal)
                .OrderBy(x => x.Key, StringComparer.Ordinal)
                .ToDictionary(x => x.Key, x => x.Count(), StringComparer.Ordinal));
            MaximumRanksByClass = new ReadOnlyDictionary<string, int>((classIds ?? Array.Empty<string>())
                .OrderBy(x => x, StringComparer.Ordinal)
                .ToDictionary(
                    x => x,
                    x => catalog.Skills.Where(s => s.IsEligible(x)).Sum(s => s.EffectiveMaximumRank(x)),
    ...
    {
        public RankedSkillJsonImportResultV1(
            RankedSkillCatalogV2 catalog,
            IEnumerable<ImportedSkillSynergyV1> importedSynergies,
            IEnumerable<string> classIds,
            IEnumerable<SkillCatalogDiagnosticV1> diagnostics,
    ...
    /// <summary>
    /// Typed authoring importer. JsonUtility is deliberately confined to this Unity adapter;
    /// all resulting gameplay definitions are engine-independent SKILL-FOUNDATION-003 objects.
    /// </summary>
    public static class RankedSkillJsonImporterV1
    {
        ...
        public static RankedSkillJsonImportResultV1 Import(string json)
        {
            ...
            {
                Error(diagnostics, "$", string.Empty, "skill-json-empty", "Skill catalog JSON is empty.");
            ...
            {
                Error(diagnostics, "$", string.Empty, "skill-json-invalid", ex.Message);
                return Failure(diagnostics);
            }
            if (dto == null)
            {
                Error(diagnostics, "$", string.Empty, "skill-json-null", "JSON produced no catalog object.");
                return Failure(diagnostics);
            }

            if (!string.Equals(dto.schemaVersion, SupportedSchemaVersion, StringComparison.Ordinal))
                Error(diagnostics, "$.schemaVersion", string.Empty, "skill-schema-unsupported", "Unsupported schema version: " + dto.schemaVersion);
            if (string.IsNullOrWhiteSpace(dto.contentVersion))
                Error(diagnostics, "$.contentVersion", string.Empty, "skill-content-version-missing", "Content version is required.");
            ...
            try
            {
                // Combined-rank synergies are intentionally omitted from the legacy V2 projector list;
                // ImportedSynergies remains authoritative for their complete gate evaluation.
                var directlyProjectable = importedSynergies.Where(x => !x.HasCombinedRequirements).Select(x => x.Definition).ToArray();
                var catalog = new RankedSkillCatalogV2(dto.schemaVersion, dto.contentVersion, skills, directlyProjectable);
            ...
            {
                Error(diagnostics, "$", string.Empty, "skill-catalog-build-failed", ex.Message);
                return Failure(diagnostics, classIds);
            }
        }
        ...
        public static SkillEffectSnapshotV2 ProjectEffects(RankedSkillJsonImportResultV1 import, RankedSkillAllocationSnapshotV2 allocation)
        {
            ...
            SkillEffectSnapshotV2 baseSnapshot = new SkillEffectProjectorV2().Project(import.Catalog, allocation);
            var contributions = baseSnapshot.Contributions.ToList();
            foreach (ImportedSkillSynergyV1 synergy in import.ImportedSynergies.Where(x => x.HasCombinedRequirements && x.IsSatisfied(allocation)))
                foreach (SkillEffectDescriptorV2 effect in synergy.Definition.Effects)
                    contributions.Add(new SkillEffectContributionV2(synergy.Definition.Id, effect));
            return new SkillEffectSnapshotV2(allocation, contributions);
        }

        private static RankedSkillDefinitionV2 BuildSkill(
            SkillDto dto,
            int index,
            HashSet<string> classIds,
            HashSet<string> categoryIds,
            Dictionary<string, HashSet<SkillModifierKindV2>> effectKinds,
            HashSet<string> skillIds,
            List<SkillCatalogDiagnosticV1> diagnostics)
        {
            ...
            {
                ClassOverrideDto item = overrideDtos[i];
                string itemPath = path + ".classOverrides[" + i + "]";
                ...
                if (item.maximumRank > 0 && (values.Length == 0 || values.Length == item.maximumRank))
                {
                    try { overrides.Add(new SkillClassOverrideV2(classId, item.maximumRank, values)); }
                    catch (Exception ex) { Error(diagnostics, itemPath, id, "skill-class-override-invalid", ex.Message); }
                }
            }
            ...
            for (int i = 0; i < milestoneDtos.Length; i++)
            {
                ...
                {
                    var effects = BuildEffects(item.effects, path + ".milestones[" + i + "].effects", id, effectKinds, false, diagnostics);
                    if (effects.Count != 0) milestones.Add(new SkillRankMilestoneV2(item.rank, effects));
                }
            }
            ...
            try
            {
                return new RankedSkillDefinitionV2(id, Clean(dto.categoryId), dto.maximumRank, dto.eligibleClassIds, prerequisites, gates, overrides, baseCurve, perRankEffects, milestones);
            }
            catch (Exception ex)
            {
                Error(diagnostics, path, id, "skill-definition-invalid", ex.Message);
                return null;
            }
        }

        private static List<ImportedSkillSynergyV1> BuildSynergies(
            SynergyDto[] source,
            IReadOnlyList<RankedSkillDefinitionV2> skills,
            HashSet<string> classIds,
            Dictionary<string, HashSet<SkillModifierKindV2>> effectKinds,
            List<SkillCatalogDiagnosticV1> diagnostics)
        {
            ...
            {
                SynergyDto dto = items[i];
                string path = "$.synergies[" + i + "]";
                ...
                RequirementDto[] requirementDtos = dto.requirements ?? Array.Empty<RequirementDto>();
                for (int r = 0; r < requirementDtos.Length; r++)
                {
                    RequirementDto item = requirementDtos[r];
                    RankedSkillDefinitionV2 skill;
                    if (item == null || !skillMap.TryGetValue(Clean(item.skillId), out skill)) Error(diagnostics, path + ".requirements[" + r + "]", id, "skill-synergy-requirement-unknown", "Unknown synergy skill.");
                    else if (item.minimumRank < 1 || item.minimumRank > MaxRank(skill)) Error(diagnostics, path + ".requirements[" + r + "].minimumRank", id, "skill-synergy-requirement-impossible", "Synergy minimum rank is impossible.");
                    else requirements.Add(new SkillSynergyRequirementV2(skill.Id, item.minimumRank));
                }

                var combined = new List<CombinedSkillRankRequirementV1>();
                CombinedRequirementDto[] combinedDtos = dto.combinedRankRequirements ?? Array.Empty<CombinedRequirementDto>();
                for (int c = 0; c < combinedDtos.Length; c++)
                {
                    CombinedRequirementDto item = combinedDtos[c];
                    string itemPath = path + ".combinedRankRequirements[" + c + "]";
                    if (item == null || item.skillIds == null || item.skillIds.Length < 2 || item.skillIds.Any(x => !skillMap.ContainsKey(Clean(x))))
                        Error(diagnostics, itemPath, id, "skill-synergy-combined-invalid", "Combined-rank requirement references missing skills.");
                    else if (item.minimumCombinedRanks < 2 || item.minimumCombinedRanks > item.skillIds.Sum(x => MaxRank(skillMap[Clean(x)])))
                        Error(diagnostics, itemPath + ".minimumCombinedRanks", id, "skill-synergy-combined-impossible", "Combined-rank requirement is impossible.");
                    else combined.Add(new CombinedSkillRankRequirementV1(item.skillIds.Select(Clean), item.minimumCombinedRanks));
                }
                ...
                foreach (SkillEffectDescriptorV2 effect in effects.Where(x => string.Equals(x.StatId, "reward.legendary_definition_weight_multiplier", StringComparison.Ordinal)))
                    if (effect.Kind != SkillModifierKindV2.Percentage || effect.Value < 0m || effect.Value > MaximumLegendaryRelativeBonus)
                        Error(diagnostics, path + ".effects", id, "skill-legendary-relative-bonus-invalid", "Legendary definition weight bonus must be a relative Percentage from 0 through 0.20.");

                if (diagnostics.Any(x => x.DefinitionId == id && x.Severity == SkillCatalogDiagnosticSeverityV1.Error)) continue;
                try
                {
                    var definition = new SkillSynergyDefinitionV2(id, requirements, effects);
                    output.Add(new ImportedSkillSynergyV1(definition, dto.displayName, dto.description, dto.eligibleClassIds, combined));
                }
        ...
        private static List<SkillEffectDescriptorV2> BuildEffects(
            EffectDto[] source,
            string path,
            string definitionId,
            Dictionary<string, HashSet<SkillModifierKindV2>> registry,
            bool requireRankValueSource,
            List<SkillCatalogDiagnosticV1> diagnostics)
        {
            ...
            for (int i = 0; i < items.Length; i++)
            {
                ...
                if (requireRankValueSource && !string.Equals(dto.valueSource, "rankValue", StringComparison.Ordinal))
                { Error(diagnostics, itemPath + ".valueSource", definitionId, "skill-effect-rank-source-required", "Per-rank effects must use rankValue."); continue; }
        ...
        {
            if (dto == null || dto.count != expectedCount)
            {
                Error(diagnostics, path, id, "skill-rank-curve-count-invalid", "Rank curve count must equal maximumRank.");
        ...
        private static Dictionary<string, HashSet<SkillModifierKindV2>> BuildEffectRegistry(EffectRegistryDto[] source, List<SkillCatalogDiagnosticV1> diagnostics)
        {
            ...
            for (int i = 0; i < items.Length; i++)
            {
                ...
                foreach (string value in dto.allowedKinds ?? Array.Empty<string>())
                {
                    SkillModifierKindV2 kind;
                    if (Enum.TryParse(value, false, out kind)) kinds.Add(kind);
                    else Error(diagnostics, "$.effects[" + i + "].allowedKinds", id, "skill-effect-kind-unknown", "Unknown modifier kind: " + value);
                }
        ...
        private static HashSet<string> UniqueIds(NamedIdDto[] source, string path, string kind, List<SkillCatalogDiagnosticV1> diagnostics)
        {
            ...
            NamedIdDto[] items = source ?? Array.Empty<NamedIdDto>();
            for (int i = 0; i < items.Length; i++)
            {
                string id = Clean(items[i]?.id);
                if (string.IsNullOrEmpty(id) || !output.Add(id)) Error(diagnostics, path + "[" + i + "].id", id, "skill-" + kind + "-id-invalid", kind + " ids must be non-empty and unique.");
            }
            return output;
        }
        ...
        private static string BuildCanonical(CatalogDto dto, IEnumerable<RankedSkillDefinitionV2> skills, IEnumerable<ImportedSkillSynergyV1> synergies, IEnumerable<string> classes, IEnumerable<string> categories, IEnumerable<string> effects)
        {
            var lines = new List<string>
            {
                ...
                dto.contentVersion ?? string.Empty,
                dto.status ?? string.Empty,
                "classes=" + string.Join(",", classes.OrderBy(x => x, StringComparer.Ordinal)),
                "categories=" + string.Join(",", categories.OrderBy(x => x, StringComparer.Ordinal)),
                "effects=" + string.Join(",", effects.OrderBy(x => x, StringComparer.Ordinal))
            };
            lines.AddRange(skills.OrderBy(x => x.Id, StringComparer.Ordinal).Select(x =>
                "skill=" + x.Id + "|" + x.CategoryId + "|" + x.MaximumRank + "|" + string.Join(",", x.RankValues.Select(v => v.ToString(CultureInfo.InvariantCulture)))));
            lines.AddRange(synergies.OrderBy(x => x.Definition.Id, StringComparer.Ordinal).Select(x =>
                "synergy=" + x.Definition.Id + "|" + string.Join(",", x.Definition.Requirements.Select(r => r.SkillId + ">=" + r.MinimumRank)) + "|" + string.Join(",", x.CombinedRequirements.Select(r => r.Canonical))));
            return string.Join("\n", lines);
        }
