...
namespace ShooterMover.UnityAdapters.Enemies.Presentation
{
    ...
    /// movement intent, wind-up and emission timing through IEnemyPresentationCommandSink2D;
    /// it never schedules attacks, emits projectiles, applies damage or owns replay admission.
    /// </summary>
    [DisallowMultipleComponent]
    internal sealed class EnemyAttackPresentationProjection2D : MonoBehaviour
    {
        ...
        private IEnemyPresentationCommandSink2D sink;
        ...
        public void Bind(RoomEnemyActor2D configuredActor, long lifecycleRevision)
        {
            ...
            IEnemyPresentationCommandSink2D resolvedSink;
        ...
        public bool TryCreatePlan(
            ...
            out EnemyAttackPresentationPlanV1 plan,
            out string diagnostic)
        {
            ...
            var pulses = new List<EnemyAttackPresentationPulseV1>(
            ...
            for (int index = 0; index < sequence.Emissions.Count; index++)
            {
                ...
                pulses.Add(new EnemyAttackPresentationPulseV1(
                    emission.EmissionStableId,
                    delay));
            }

            pulses.Sort(EnemyAttackPresentationPulseV1.Compare);
            plan = new EnemyAttackPresentationPlanV1(
        ...
        public void Apply(EnemyAttackPresentationPlanV1 plan)
        {
            ...
            {
                EnemyAttackPresentationPulseV1 pulse = plan.Pulses[index];
        ...
        public void Rollback(EnemyAttackPresentationPlanV1 plan)
        {
            if (plan == null) return;
        ...
        public void Cancel(EnemyAttackSequenceCancellationFactV1 cancellation)
        {
        ...
        private static void ResolveExactlyOneSink(
            ...
            out IEnemyPresentationCommandSink2D sink)
        {
            sinkComponent = null;
            sink = null;
            int count = 0;
            ...
            for (int index = 0; index < behaviours.Length; index++)
            {
                ...
                IEnemyPresentationCommandSink2D candidate =
                    behaviour as IEnemyPresentationCommandSink2D;
    ...
    internal sealed class EnemyAttackPresentationPlanV1
    {
        public EnemyAttackPresentationPlanV1(
            StableId dispatchStableId,
            Vector2 committedFacing,
            float firstDelaySeconds,
            IReadOnlyList<EnemyAttackPresentationPulseV1> pulses)
        {
        ...
        public IReadOnlyList<EnemyAttackPresentationPulseV1> Pulses { get; }
    }

    internal sealed class EnemyAttackPresentationPulseV1
    {
        public EnemyAttackPresentationPulseV1(
        ...
        public static int Compare(
            EnemyAttackPresentationPulseV1 left,
            EnemyAttackPresentationPulseV1 right)
        {
