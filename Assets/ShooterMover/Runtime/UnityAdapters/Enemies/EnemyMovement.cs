...
namespace ShooterMover.UnityAdapters.Enemies
{
    ...
    /// </summary>
    public interface IEnemyMoveSource
    {
        bool TryDecide(
    ...
    public enum EnemyFixedStepStatus
    {
        Applied = 1,
        AppliedClamped = 2,
        NotConfigured = 3,
    ...
    public sealed class EnemyFixedStepResult
    {
        internal EnemyFixedStepResult(
            EnemyFixedStepStatus status,
            long fixedStep,
            EnemyMove decision,
        ...
        public EnemyFixedStepStatus Status { get; }

        public long FixedStep { get; }

        public EnemyMove Decision { get; }
        ...
        public bool Applied
        {
            get
            {
                return Status == EnemyFixedStepStatus.Applied
                    || Status == EnemyFixedStepStatus.AppliedClamped;
            }
        }
    }
    ...
    public sealed class EnemyMovement : MonoBehaviour
    {
        ...
        private IEnemyMoveSource decisionSource;
        private IEnemyTargetSource targetSource;
        private EnemyContact contactAdapter;
        ...
        public void Configure(
            Rigidbody2D body,
            IEnemyState actorAuthority,
            IEnemyMoveSource actorDecisionSource,
            IEnemyTargetSource actorTargetSource,
            EnemyContact actorContactAdapter,
            double maximumMovementSpeed)
        {
            ...
            if (!actorContactAdapter.IsConfigured)
            {
                throw new InvalidOperationException(
                    "EnemyContact must be configured before the actor adapter.");
            }
            ...
            if (IsConfigured)
            {
                ...
                throw new InvalidOperationException(
                    "EnemyMovement is already configured with different dependencies.");
            }
        ...
        public bool Activate()
        {
            ...
            {
                throw new InvalidOperationException(
                    "EnemyMovement must be configured before activation.");
            }
        ...
        /// </summary>
        public EnemyFixedStepResult ExecuteFixedStep(double deltaTimeSeconds)
        {
            ...
            if (!IsConfigured)
            {
                return Result(EnemyFixedStepStatus.NotConfigured, step, null, 0d, 0d);
            }

            if (!IsActive)
            {
                ClearEnemyVelocity();
                return Result(EnemyFixedStepStatus.AdapterInactive, step, null, 0d, 0d);
            }
            ...
            try
            {
                if (!authority.TryReadState(out state) || state == null)
                {
                    ...
                    AdvanceFixedStep();
                    return Result(
                        EnemyFixedStepStatus.AuthorityUnavailable,
                        step,
                        null,
                        0d,
                        0d);
            ...
            catch (ArgumentException)
            {
                ClearEnemyVelocity();
                AdvanceFixedStep();
                return Result(EnemyFixedStepStatus.AuthorityUnavailable, step, null, 0d, 0d);
            }
            catch (InvalidOperationException)
            {
                ClearEnemyVelocity();
            ...
            if (!state.IsActive)
            {
                ClearEnemyVelocity();
                AdvanceFixedStep();
                return Result(EnemyFixedStepStatus.ActorInactive, step, null, 0d, 0d);
            }
            ...
            try
            {
                if (!targetSource.TryReadTarget(out target) || target == null)
                {
                    ClearEnemyVelocity();
                    AdvanceFixedStep();
                    return Result(EnemyFixedStepStatus.TargetUnavailable, step, null, 0d, 0d);
                }
            }
            ...
            try
            {
                    ...
                    || decision == null)
                {
                    ClearEnemyVelocity();
                    AdvanceFixedStep();
                    return Result(EnemyFixedStepStatus.DecisionUnavailable, step, null, 0d, 0d);
                }
            }
            catch (ArgumentException)
            {
                ClearEnemyVelocity();
                AdvanceFixedStep();
                return Result(EnemyFixedStepStatus.InvalidDecision, step, null, 0d, 0d);
            }
            catch (InvalidOperationException)
            {
                ClearEnemyVelocity();
            ...
            if (decision.ActorId != state.ActorId || decision.TargetId != target.TargetId)
            {
                ClearEnemyVelocity();
                AdvanceFixedStep();
                return Result(EnemyFixedStepStatus.InvalidDecision, step, decision, 0d, 0d);
            }
            ...
            try
            {
                ...
                {
                    AdvanceFixedStep();
                    return Result(EnemyFixedStepStatus.BodyUnavailable, step, decision, 0d, 0d);
                }
            ...
            return Result(
                clamped
                    ? EnemyFixedStepStatus.AppliedClamped
                    : EnemyFixedStepStatus.Applied,
                step,
                decision,
                velocityX,
                velocityY);
        ...
        private static EnemyFixedStepResult Result(
            EnemyFixedStepStatus status,
            long fixedStep,
            EnemyMove decision,
            double velocityX,
            double velocityY)
        {
            return new EnemyFixedStepResult(
                status,
                fixedStep,
                decision,
                velocityX,
