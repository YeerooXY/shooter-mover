...
namespace ShooterMover.RunPickups
{
    ...
    /// </summary>
    public sealed partial class RunLocalPickupState : IRunPickupCollectionState
    {
        private sealed class CollectionReplayRecord
        {
            public CollectionReplayRecord(
                string commandFingerprint,
                RunPickupCollectionResult result)
            {
            ...
            public RunPickupCollectionResult Result { get; }
        }

        private readonly object gate = new object();
        private readonly IRunPickupRunSessionPort runSession;
        private readonly IRunPickupSourcePositionPort sourcePositions;
        private readonly Dictionary<StableId, RunPickupSnapshot> byPickup =
            new Dictionary<StableId, RunPickupSnapshot>();
        ...
        public RunLocalPickupState(
            IRunPickupRunSessionPort runSession,
        ...
        public int AvailablePickupCount
        {
            get
            {
                lock (gate)
                {
                    return CountCurrentLifecycle(RunPickupState.Available);
                }
            }
        }
        ...
        public int CollectedPickupCount
        {
            get
            {
                lock (gate)
                {
                    return CountCurrentLifecycle(RunPickupState.Collected);
                }
            }
        }
            ...
            RunPickupGeneratedBatch batch)
        {
            ...
            lock (gate)
            {
                ...
                {
                    RunPickupGeneratedReward reward = batch.GeneratedRewards[index];
                    StableId pickupId = RunPickupIdentity.DerivePickupStableId(
                        batch,
                        reward);
                    ...
                    RunPickupSnapshot existingPickup;
                ...
                if (!positionResolved || worldSpawnContext == null)
                {
                    ...
                    for (int index = 0; index < batch.GeneratedRewards.Count; index++)
                    {
                        ...
                        RunPickupSnapshot existing;
                        if (byPickup.TryGetValue(pickupId, out existing))
                        {
                            continue;
                        }

                        RunPickupSnapshot pending = CreatePendingSnapshot(
                            batch,
                            reward,
                            pickupId,
                ...
                for (int index = 0; index < batch.GeneratedRewards.Count; index++)
                {
                    ...
                    if (!byPickup.TryGetValue(pickupId, out existing))
                    {
                        RunPickupSnapshot available = new RunPickupSnapshot(
                            pickupId,
                            batch,
                            reward,
                    ...
                    else if (existing.State == RunPickupState.PendingSourcePosition)
                    {
