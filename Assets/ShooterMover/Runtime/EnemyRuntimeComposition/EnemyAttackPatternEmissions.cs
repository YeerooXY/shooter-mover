...
{
    public enum EnemyAttackEffectEmissionKind
    {
        Projectile = 1,
        MeleeStrike = 2,
    }
    ...
    /// </summary>
    public sealed class EnemyAttackEffectEmission
    {
        internal EnemyAttackEffectEmission(
            StableId emissionStableId,
            StableId sequenceStableId,
            string sequenceFingerprint,
            EnemyAttackEffectEmissionKind kind,
            ...
            EnemyAttackScheduledMeleeStrike meleeStrike)
        {
            ...
            if (!Enum.IsDefined(typeof(EnemyAttackEffectEmissionKind), kind))
            ...
            if ((kind == EnemyAttackEffectEmissionKind.Projectile) != (projectile != null))
                throw new ArgumentException(
                    "Projectile emissions must carry exactly one projectile fact.");
            if ((kind == EnemyAttackEffectEmissionKind.MeleeStrike) != (meleeStrike != null))
                throw new ArgumentException(
        ...
        public EnemyAttackEffectEmissionKind Kind { get; }
        public EnemyAttackExecutionRequest Execution { get; }
        public double ScheduledAtSeconds { get; }
        public double ActiveUntilSeconds { get; }
        public EnemyAttackScheduledProjectile Projectile { get; }
        public EnemyAttackScheduledMeleeStrike MeleeStrike { get; }
    ...
    public static class EnemyAttackEffectEmissionProjector
    {
        public static IReadOnlyList<EnemyAttackEffectEmission> Project(
            EnemyAttackExecutionRequest execution,
            EnemyAttackSequence sequence)
        {
            ...
            var emissions = new List<EnemyAttackEffectEmission>();
            for (int index = 0; index < sequence.Projectiles.Count; index++)
            {
                EnemyAttackScheduledProjectile projectile = sequence.Projectiles[index];
                emissions.Add(new EnemyAttackEffectEmission(
                    projectile.ProjectileStableId,
                    sequence.Identity.SequenceStableId,
                    sequence.Fingerprint,
                    EnemyAttackEffectEmissionKind.Projectile,
                    execution,
            ...
            {
                EnemyAttackScheduledMeleeStrike strike = sequence.MeleeStrikes[index];
                emissions.Add(new EnemyAttackEffectEmission(
            ...
            return new ReadOnlyCollection<EnemyAttackEffectEmission>(emissions);
        }
    }
}
