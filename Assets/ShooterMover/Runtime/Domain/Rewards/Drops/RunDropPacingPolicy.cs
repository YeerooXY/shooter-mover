...
{
    public sealed class DropSaturationBand : IComparable<DropSaturationBand>
    {
        public DropSaturationBand(int minimumBoxCount, int multiplierMillionths)
        {
            if (minimumBoxCount < 0) throw new ArgumentOutOfRangeException(nameof(minimumBoxCount));
            if (multiplierMillionths < 1 || multiplierMillionths > RunDropPacingPolicy.ProbabilityScale)
        ...
        public int CompareTo(DropSaturationBand other) { return ReferenceEquals(other, null) ? 1 : MinimumBoxCount.CompareTo(other.MinimumBoxCount); }
    ...
    /// </summary>
    public sealed class RunDropPacingPolicy
    {
        public const int ProbabilityScale = 1000000;
        private readonly ReadOnlyCollection<DropSaturationBand> roomBands;
        private readonly ReadOnlyCollection<DropSaturationBand> runBands;
        private readonly string canonicalText;

        public RunDropPacingPolicy(
            ...
            IEnumerable<DropSaturationBand> roomBands,
            IEnumerable<DropSaturationBand> runBands)
        {
        ...
        public IReadOnlyList<DropSaturationBand> RoomBands { get { return roomBands; } }
        public IReadOnlyList<DropSaturationBand> RunBands { get { return runBands; } }
        ...
        public int CalculateEffectiveRandomBoxProbability(int baseProbabilityMillionths, ParticipantDropPacingState state)
        {
        ...
        private static int ResolveBand(IReadOnlyList<DropSaturationBand> bands, int count)
        {
        ...
        private static ReadOnlyCollection<DropSaturationBand> CopyBands(IEnumerable<DropSaturationBand> source, string parameterName)
        {
            if (source == null) throw new ArgumentNullException(parameterName);
            var copy = new List<DropSaturationBand>();
            foreach (DropSaturationBand band in source)
            {
            ...
            return new ReadOnlyCollection<DropSaturationBand>(copy);
        }

        private static void AppendBands(StringBuilder builder, string prefix, IReadOnlyList<DropSaturationBand> bands)
        {
