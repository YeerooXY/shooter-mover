...
{
    /// <summary>
    /// Complete immutable identity and balance context for one participant roll.
    /// The logical operation identity is derived only from stable terminal/participant
    /// identity. The complete context fingerprint is compared separately so a changed
    /// immutable fact, profile fingerprint, modifier or seed rejects as a conflict.
    /// </summary>
    public sealed class PersonalRewardRollContext
    {
        private readonly ReadOnlyCollection<StableId> eventModifierIds;
        private readonly string canonicalText;

        public PersonalRewardRollContext(
            StableId runStableId,
            int runLifecycleGeneration,
            StableId terminalSourceStableId,
            int sourceLifecycleGeneration,
            ...
            RewardProfileResolution profileResolution,
            ...
            int algorithmVersion)
        {
            ...
            TerminalSourceStableId = terminalSourceStableId
                ?? throw new ArgumentNullException(nameof(terminalSourceStableId));
            ...
            if (string.IsNullOrWhiteSpace(terminalFactFingerprint))
            {
                throw new ArgumentException(
                    "The immutable terminal-fact fingerprint is required.",
                    nameof(terminalFactFingerprint));
            }
            ...
            ParticipantEligible = participantEligible;
            ...
            TerminalFactFingerprint = terminalFactFingerprint.Trim();
            RootSeed = rootSeed;
            AlgorithmVersion = algorithmVersion;

            OperationStableId = RewardGenerationFingerprint.DeriveStableId(
                "personalrewardoperation",
                RunStableId.ToString(),
                RunLifecycleGeneration.ToString(CultureInfo.InvariantCulture),
                TerminalSourceStableId.ToString(),
                SourceLifecycleGeneration.ToString(CultureInfo.InvariantCulture),
                RoomStableId.ToString(),
                RoomLifecycleGeneration.ToString(CultureInfo.InvariantCulture),
                PlacementStableId.ToString(),
                ParticipantStableId.ToString(),
                ProfileResolution.DeclaredProfileReferenceId.ToString(),
                AlgorithmVersion.ToString(CultureInfo.InvariantCulture));

            var builder = new StringBuilder(
                "schema=personal-reward-roll-context-v1");
            builder.Append("\noperation_id=").Append(OperationStableId)
                .Append("\nrun_id=").Append(RunStableId)
                .Append("\nrun_lifecycle=")
                .Append(RunLifecycleGeneration.ToString(CultureInfo.InvariantCulture))
                .Append("\nterminal_source_id=").Append(TerminalSourceStableId)
                .Append("\nsource_lifecycle=")
                .Append(SourceLifecycleGeneration.ToString(CultureInfo.InvariantCulture))
                .Append("\nroom_id=").Append(RoomStableId)
                .Append("\nroom_lifecycle=")
                .Append(RoomLifecycleGeneration.ToString(CultureInfo.InvariantCulture))
                .Append("\nplacement_id=").Append(PlacementStableId)
                .Append("\nparticipant_id=").Append(ParticipantStableId)
                .Append("\nparticipant_eligible=")
                .Append(ParticipantEligible ? "1" : "0")
                .Append("\nplayer_level=")
                ...
                .Append("\nmoney_quantity_multiplier_permille=")
                .Append(MoneyQuantityMultiplierPermille.ToString(CultureInfo.InvariantCulture))
                .Append("\nscrap_quantity_multiplier_permille=")
                .Append(ScrapQuantityMultiplierPermille.ToString(CultureInfo.InvariantCulture))
                .Append("\nprofile_resolution=").Append(ProfileResolution.Fingerprint)
                .Append("\npacing_policy=").Append(PacingPolicy.Fingerprint)
                .Append("\nterminal_fact_fingerprint=").Append(TerminalFactFingerprint)
                .Append("\nroot_seed=")
                .Append(RootSeed.ToString(CultureInfo.InvariantCulture))
                .Append("\nalgorithm_version=")
                .Append(AlgorithmVersion.ToString(CultureInfo.InvariantCulture))
                .Append("\nevent_modifier_count=")
                .Append(this.eventModifierIds.Count.ToString(CultureInfo.InvariantCulture));
        ...
        public RewardProfileResolution ProfileResolution { get; }
        public RunDropPacingPolicy PacingPolicy { get; }
        public string TerminalFactFingerprint { get; }
        public ulong RootSeed { get; }
        public int AlgorithmVersion { get; }
        public StableId OperationStableId { get; }
        public string Fingerprint { get; }

        public IEnumerable<StableId> EnumerateTierSelectionContexts()
        {
            yield return DifficultyStableId;
            yield return GameModeStableId;
            for (int index = 0; index < eventModifierIds.Count; index++)
            {
                yield return eventModifierIds[index];
            }
        }
        ...
        {
            var set = new SortedSet<StableId>();
            if (source != null)
            {
            ...
            return new ReadOnlyCollection<StableId>(new List<StableId>(set));
        }
    }
}
