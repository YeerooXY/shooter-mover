...
{
    /// <summary>
    /// Structural validation for GunLiveProfile v1. These bounds reject malformed
    /// runtime tuning without choosing final balance for any gun package.
    /// </summary>
    public static class GunLiveProfileValidator
    {
        ...
        {
            if (profileVersion != GunLiveProfile.CurrentProfileVersion)
            {
                throw new NotSupportedException(
                    "Unsupported gun runtime profile version: " + profileVersion + ".");
            }
            ...
            double chargeSeconds)
        {
            ...
            if (cycleMode == GunCycleMode.Heat)
            {
                if (chargeSeconds != 0d)
                {
                    throw new ArgumentException(
