...
{
    public enum GunBehaviorModuleExecutionStatus
    {
        Succeeded = 1,
        Empty = 2,
        Faulted = 3,
    ...
    public enum GunBehaviorModuleFaultKind
    {
        None = 0,
        ModuleException = 1,
        NullPlan = 2,
    ...
    /// </summary>
    public sealed class GunFireExecutionOperationEntry
    {
        internal GunFireExecutionOperationEntry(
        ...
        public IGunFireExecutionOperation Operation { get; }
    ...
    /// </summary>
    public sealed class GunBehaviorModuleExecution
    {
        internal GunBehaviorModuleExecution(
            StableId moduleId,
            ...
            int operationCount)
        {
            ...
            if (!Enum.IsDefined(typeof(GunBehaviorModuleExecutionStatus), status))
            {
                throw new ArgumentOutOfRangeException(nameof(status));
            }

            if (!Enum.IsDefined(typeof(GunBehaviorModuleFaultKind), faultKind))
            {
            ...
            if (status == GunBehaviorModuleExecutionStatus.Faulted)
            {
                if (faultKind == GunBehaviorModuleFaultKind.None || operationCount != 0)
                {
                    throw new ArgumentException(
            ...
            {
                if (faultKind != GunBehaviorModuleFaultKind.None)
                {
                    throw new ArgumentException(
                ...
                if (status == GunBehaviorModuleExecutionStatus.Empty && operationCount != 0)
                {
                    throw new ArgumentException(
                ...
                if (status == GunBehaviorModuleExecutionStatus.Succeeded && operationCount == 0)
                {
                    throw new ArgumentException(
        ...
        public GunBehaviorModuleExecutionStatus Status { get; }

        public GunBehaviorModuleFaultKind FaultKind { get; }
    ...
    /// validated GunLiveProfile to one fire input.
    /// </summary>
    public sealed class GunFireExecutionPlan : IEquatable<GunFireExecutionPlan>
    {
        ...
        public const string DeterministicIdentityNamespace = "gun-plan";

        private const uint FnvOffsetBasis = 2166136261u;
        private const uint FnvPrime = 16777619u;

        private readonly GunFireExecutionOperationEntry[] operations;
        private readonly GunBehaviorModuleExecution[] moduleExecutions;
        private readonly string canonicalText;

        internal GunFireExecutionPlan(
            GunBehaviorInput input,
            GunFireExecutionOperationEntry[] operations,
            GunBehaviorModuleExecution[] moduleExecutions)
        {
            if (input == null)
            {
            ...
            this.operations = (GunFireExecutionOperationEntry[])operations.Clone();
            this.moduleExecutions = (GunBehaviorModuleExecution[])moduleExecutions.Clone();
        ...
        public GunBehaviorInput Input { get; }
        ...
        public int FaultCount
        {
            get
            {
                ...
                {
                    if (moduleExecutions[index].Status == GunBehaviorModuleExecutionStatus.Faulted)
                    {
                        count++;
                    }
                }
        ...
        public GunFireExecutionOperationEntry GetOperation(int index)
        {
        ...
        public GunBehaviorModuleExecution GetModuleExecution(int index)
        {
        ...
        public bool Equals(GunFireExecutionPlan other)
        {
        ...
        {
            return Equals(obj as GunFireExecutionPlan);
        }
            ...
            GunBehaviorModuleExecution[] moduleExecutions)
        {
            ...
            {
                GunBehaviorModuleExecution execution = moduleExecutions[moduleIndex];
                if (execution == null)
                {
                    throw new ArgumentException(
                ...
                for (int localIndex = 0; localIndex < execution.OperationCount; localIndex++)
                {
                    ...
                    GunFireExecutionOperationEntry entry = operations[operationIndex];
                    if (entry == null)
                    {
                        throw new ArgumentException(
        ...
        private string BuildCanonicalText()
        {
            ...
            for (int index = 0; index < moduleExecutions.Length; index++)
            {
                GunBehaviorModuleExecution execution = moduleExecutions[index];
            ...
            {
                GunFireExecutionOperationEntry operation = operations[index];
        ...
        private static string FormatStatus(GunBehaviorModuleExecutionStatus status)
        {
            switch (status)
            {
                case GunBehaviorModuleExecutionStatus.Succeeded:
                    return "succeeded";
                case GunBehaviorModuleExecutionStatus.Empty:
                    return "empty";
                case GunBehaviorModuleExecutionStatus.Faulted:
                    return "faulted";
                default:
                    throw new ArgumentOutOfRangeException(nameof(status));
            }
        ...
        private static string FormatFault(GunBehaviorModuleFaultKind faultKind)
        {
            switch (faultKind)
            {
                case GunBehaviorModuleFaultKind.None:
                    return "none";
                case GunBehaviorModuleFaultKind.ModuleException:
                    return "module-exception";
                case GunBehaviorModuleFaultKind.NullPlan:
                    return "null-plan";
                case GunBehaviorModuleFaultKind.ModuleIdMismatch:
                    return "module-id-mismatch";
                case GunBehaviorModuleFaultKind.DuplicateOperationId:
                    return "duplicate-operation-id";
                case GunBehaviorModuleFaultKind.PlanLimitExceeded:
