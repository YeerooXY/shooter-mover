...
{
    /// <summary>
    /// Declares the mutually exclusive cycle-resource model used by one gun mount.
    /// </summary>
    public enum GunCycleMode
    {
        None = 1,
        Heat = 2,
        Charge = 3,
    ...
    /// <summary>
    /// Immutable, engine-independent runtime tuning for one gun used by the accepted
    /// four-mount combat model. Runtime state remains independent per mount.
    /// </summary>
    public sealed class GunLiveProfile : IEquatable<GunLiveProfile>
    {
        ...
        public const string DeterministicIdentityNamespace = "gun-runtime";
        ...
        /// <summary>
        /// Minimum authored interval between gun cycles.
        /// </summary>
        ...
        public static GunLiveProfile Create(
            int profileVersion,
            StableId profileId,
            double cadenceSeconds,
            int burstShotCount,
            ...
            int presentationPriority)
        {
            ...
            GunLiveProfileValidator.ValidateValues(
                profileVersion,
                profileId,
                cadenceSeconds,
                burstShotCount,
        ...
        public static GunLiveProfile ParseCanonical(
            string text,
            IEnumerable<StableId> knownBehaviorModuleIds)
        {
            if (text == null)
            ...
            if (text.Length == 0)
            {
                throw new FormatException("Gun runtime profile cannot be empty.");
            }

            if (text.IndexOf('\r') >= 0 || text.EndsWith("\n", StringComparison.Ordinal))
            {
                throw new FormatException(
                    "Canonical gun runtime profile text must use LF separators and no trailing newline.");
            }

            string[] lines = text.Split('\n');
            if (lines.Length < CanonicalFixedFieldCount + 1)
            {
                throw new FormatException("Gun runtime profile is missing required canonical fields.");
            }
            ...
            if (behaviorModuleCount < 0
                || behaviorModuleCount > GunLiveProfileValidator.MaximumBehaviorModuleCount)
            {
            ...
            {
                throw new FormatException(
                    "Gun runtime profile does not contain the declared number of behavior modules.");
            }
            ...
            {
                throw new FormatException(
                    "Gun runtime profile is valid but not in canonical form.");
            }
        ...
        public bool Equals(GunLiveProfile other)
        {
        ...
        {
            return Equals(obj as GunLiveProfile);
        }
        ...
        public static bool operator ==(GunLiveProfile left, GunLiveProfile right)
        {
        ...
        private static string FormatCycleMode(GunCycleMode cycleMode)
        {
            switch (cycleMode)
            {
                case GunCycleMode.None:
                    return "none";
                case GunCycleMode.Heat:
                    return "heat";
                case GunCycleMode.Charge:
                    return "charge";
                default:
