...
namespace ShooterMover.Domain.Combat
{
    ...
    /// on gun IDs or operation kinds.
    /// </summary>
    public interface IGunFireExecutionOperation
    {
        /// <summary>
    ...
    /// <summary>
    /// Immutable validated per-fire input shared by every module in one gun profile.
    /// Geometry is represented as plain scalar values so the domain remains Unity-free.
    /// </summary>
    public sealed class GunBehaviorInput
    {
        public GunBehaviorInput(
            StableId combatEventId,
            StableId gunId,
            StableId mountId,
            long simulationStep,
            GunLiveProfile runtimeProfile,
            bool isEmpowered,
            double originX,
            double originY,
            double directionX,
        ...
        public GunLiveProfile RuntimeProfile { get; }
    ...
    public sealed class GunBehaviorModulePlan
    {
            ...
            params IGunFireExecutionOperation[] operations)
        {
            ...
            this.operations = new IGunFireExecutionOperation[operations.Length];
            HashSet<StableId> operationIds = new HashSet<StableId>();
            for (int index = 0; index < operations.Length; index++)
            {
                IGunFireExecutionOperation operation = operations[index];
                if (operation == null)
                {
                    throw new ArgumentException(
        ...
        public IGunFireExecutionOperation GetOperation(int index)
        {
    ...
    /// <summary>
    /// Explicit pure-function boundary for one reusable gun behavior.
    /// Implementations must not mutate input, global state, or engine objects.
    /// </summary>
    public interface IGunBehaviorModule
    {
        StableId ModuleId { get; }

        GunBehaviorModulePlan BuildExecutionPlan(GunBehaviorInput input);
    }
}
