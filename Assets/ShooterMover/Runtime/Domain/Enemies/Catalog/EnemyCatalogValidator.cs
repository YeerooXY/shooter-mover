...
namespace ShooterMover.Domain.Enemies.Catalog
{
    public static partial class EnemyCatalogValidator
    {
        ...
        public static EnemyCatalogValidationResult Validate(
            int schemaVersion,
            StableId contentVersion,
            IEnumerable<EnemyDefinition> definitions,
            IEnemyCatalogRegistry registry)
        {
            var issues = new List<EnemyCatalogIssue>();
            if (schemaVersion < EnemyCatalog.MinimumSupportedSchemaVersion
                || schemaVersion > EnemyCatalog.SupportedSchemaVersion)
            {
                Add(
                    issues,
                    ...
                    + EnemyCatalog.MinimumSupportedSchemaVersion
                    + " and " + EnemyCatalog.SupportedSchemaVersion + ".");
            }
            ...
            if (definitions == null)
            {
                ...
                return new EnemyCatalogValidationResult(issues);
            }

            var values = new List<EnemyDefinition>();
            foreach (EnemyDefinition definition in definitions) values.Add(definition);
            ...
            {
                EnemyDefinition definition = values[index];
            ...
            int schemaVersion)
        {
            ...
            if (!Enum.IsDefined(typeof(EnemyCatalogRoomClearRole), definition.RoomClearRole))
            {
                Add(
                    issues,
