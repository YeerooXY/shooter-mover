...
namespace ShooterMover.Domain.Enemies.Catalog
{
    public sealed class EnemyCatalog
    {
        ...
        private readonly ReadOnlyCollection<EnemyDefinition> definitions;
        private readonly Dictionary<StableId, EnemyDefinition> definitionsById;
            ...
            IEnumerable<EnemyDefinition> definitions)
        {
            ...
            var ordered = new List<EnemyDefinition>();
            foreach (EnemyDefinition definition in definitions)
            {
                if (definition == null)
                    throw new ArgumentException(
                        ...
                        nameof(definitions));
                EnemyDefinition canonical = CanonicalizeDefinition(definition);
                if (schemaVersion <= 1)
                {
                    for (int attackIndex = 0;
                        ...
                        attackIndex++)
                    {
                        EnemyAttackDescriptorCompatibility.MarkLegacyCompatibility(
            ...
            definitionsById = new Dictionary<StableId, EnemyDefinition>();
            for (int index = 0; index < ordered.Count; index++)
            {
                EnemyDefinition definition = ordered[index];
            ...
            this.definitions = new ReadOnlyCollection<EnemyDefinition>(ordered);
            Fingerprint = EnemyCatalogFingerprint.BuildCatalog(
                schemaVersion,
                contentVersion,
                ordered);
        }
        ...
        public EnemyDefinition GetDefinition(StableId definitionId)
        {
        ...
        private static EnemyDefinition CanonicalizeDefinition(EnemyDefinition definition)
        {
            ...
            attacks.Sort(EnemyCatalogFingerprint.CompareAttacks);
            return definition.WithAttacks(attacks);
        }

        private static EnemyAttackCapabilityDescriptor CloneAttack(
        ...
        private static int CompareDefinitions(
            EnemyDefinition left,
            EnemyDefinition right)
        {
