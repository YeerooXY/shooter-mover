...
{
    public sealed class RetiredGunSaveMigrationResult
    {
        internal RetiredGunSaveMigrationResult(
            bool succeeded,
            bool changed,
    ...
    /// <summary>
    /// Versioned decoder and cleanup boundary for deleted gun content. Retired IDs live
    /// only here so they can be removed from old saves; they are never registered in the
    /// production gun or equipment catalogues and are never translated to current gear.
    ...
    /// </summary>
    public static class RetiredGunSaveMigration
    {
        ...
        public static RetiredGunSaveMigrationResult Migrate(
            PlayerAccountSnapshot account,
            Func<StableId> instanceIdFactory = null)
        {
            if (account == null)
            {
                return Failure("retired-gun-migration-account-null", null);
            }
            ...
            try
            {
                     ...
                     slotIndex++)
                {
                    ...
                    {
                        return Failure(
                            "retired-gun-migration-character-rejected:"
                            + slotIndex
                            + ":"
                            + diagnostic,
                            account);
                ...
                return new RetiredGunSaveMigrationResult(
                    true,
            ...
            out string diagnostic)
        {
            ...
            SavePartSnapshot loadoutComponent;
            ...
            if (!GameSaveFormats.PlayerHoldings.TryDecode(
            ...
            // gun bindings. The legacy loadout component is retained as a
            // compatibility projection for armour/non-gun slots only. A
            // previous migration could leave both representations populated,
            // which the account semantic validator correctly rejects.
            SavePartSnapshot mountV2Component;
            bool hasMountData = character.TryGetComponent(
                LoadoutSavePart.Definition()
                    .ComponentStableId,
                out mountV2Component);
            if (hasMountData)
            {
                LoadoutSnapshot mountSnapshot;
            ...
            PlayerHoldingsSnapshot currentHoldings =
                retiredInstanceIds.Count > 0
                    ...
                    : holdings;
            LegacyGunInventory repaired =
                LegacyGunSetup.Repair(
        ...
        private static RetiredGunSaveMigrationResult Failure(
            string diagnostic,
            PlayerAccountSnapshot account)
        {
            return new RetiredGunSaveMigrationResult(
