        ...
        public ShopPurchaseFact Purchase(ShopPurchaseCommand command)
        {
            lock (sync)
            {
                ...
                if (purchases.TryGetValue(command.TransactionStableId, out prior))
                {
                    ...
                    if (prior.Fact.OriginalStatus == ShopPurchaseStatus.PurchasePending
                        || prior.Fact.OriginalStatus == ShopPurchaseStatus.CompensationPending)
                    {
                ...
                {
                    return RecordTerminal(command, null, ShopPurchaseStatus.UnknownShop, 0L,
                ...
                if (!string.Equals(
                    ...
                    StringComparison.Ordinal))
                {
                    return RecordTerminal(command, null, ShopPurchaseStatus.StaleInventoryFingerprint, 0L,
                        money.Balance, money.Balance, false, "shop-inventory-fingerprint-stale");
                }

                ShopStockEntry entry = state.FindEntry(command.StockEntryStableId);
                if (entry == null)
                {
                    return RecordTerminal(command, null, ShopPurchaseStatus.UnknownStockEntry, 0L,
                        money.Balance, money.Balance, false, "shop-stock-entry-unknown");
                }

                if (entry.State != ShopStockEntryState.Available)
                {
                    return RecordTerminal(command, entry, ShopPurchaseStatus.SoldOut, entry.Price,
                ...
                {
                    return RecordTerminal(command, entry, ShopPurchaseStatus.PriceMismatch, entry.Price,
                ...
                {
                    return RecordTerminal(command, entry, ShopPurchaseStatus.InsufficientFunds,
                ...
                if (!IsCommitAccepted(committed.Status))
                {
                    ...
                    return RecordTerminal(command, entry, ShopPurchaseStatus.RewardApplicationRejected,
                ...
                {
                    state.SetEntry(entry);
                    ShopPurchaseStatus status = spend.Status == MoneyWalletTransactionStatus.InsufficientFunds
                ...
                if (IsRewardApplied(claimed.Status))
                {
                    ...
                    state.SetEntry(entry.WithPurchaseState(
                        ...
                        command.TransactionStableId));
                    return RecordTerminal(command, entry, ShopPurchaseStatus.Applied,
                        entry.Price, balanceBefore, money.Balance, true, null);
                }

                if (claimed.Status == RewardApplicationResultStatus.ClaimedPendingApplication)
                {
