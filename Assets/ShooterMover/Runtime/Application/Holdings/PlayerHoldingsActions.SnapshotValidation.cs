...
namespace ShooterMover.Application.Holdings
{
    ...
    {
        private PlayerHoldingsImportResult ImportFailure(
        ...
        private bool TryValidateTransactionRecords(
            ...
            out Dictionary<StableId, PlayerHoldingsTransactionRecord> records,
            out string rejectionCode)
        {
            ...
            {
                PlayerHoldingsTransactionRecord record =
        ...
        private static PlayerHoldingsMutationStatus MapImportedOriginalStatus(
        ...
        private bool TryRebuildAppliedState(
            PlayerHoldingsSnapshot snapshot,
            out Dictionary<StableId, UniqueHoldingSnapshot> rebuiltUnique,
            out Dictionary<StableId, UniqueIdentityHistory> rebuiltUniqueHistory,
            out Dictionary<StableId, StackState> rebuiltStacks,
            out Dictionary<StableId, RewardGrantKind> rebuiltStackHistory,
            out string rejectionCode)
        {
            rebuiltUnique =
                new Dictionary<StableId, UniqueHoldingSnapshot>();
            rebuiltUniqueHistory =
                new Dictionary<StableId, UniqueIdentityHistory>();
            rebuiltStacks = new Dictionary<StableId, StackState>();
            rebuiltStackHistory =
                new Dictionary<StableId, RewardGrantKind>();

            var applied = new List<PlayerHoldingsTransactionRecord>();
            for (int index = 0; index < snapshot.Transactions.Count; index++)
            {
                PlayerHoldingsTransactionRecord record =
                    snapshot.Transactions[index];
            ...
            applied.Sort(delegate(
                PlayerHoldingsTransactionRecord left,
                PlayerHoldingsTransactionRecord right)
            {
            ...
            {
                PlayerHoldingsTransactionRecord record = applied[index];
        ...
        private static long GetRebuiltQuantity(
            ...
            IDictionary<StableId, UniqueHoldingSnapshot> rebuiltUnique,
        ...
        private static long CommitRebuiltMutation(
            ...
            IDictionary<StableId, StackState> rebuiltStacks,
            IDictionary<StableId, RewardGrantKind> rebuiltStackHistory)
        {
            EconomyTransactionCommand transaction = command.Transaction;
            ...
            IDictionary<StableId, StackState> rebuiltStacks)
        {
            ...
            {
                UniqueHoldingSnapshot expected =
                    snapshot.UniqueHoldings[index];
                UniqueHoldingSnapshot actual;
