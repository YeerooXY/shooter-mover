...
namespace ShooterMover.Application.Rewards.Generation
{
    ...
    {
        private static EquipmentGenerationResult BuildEquipmentFailure(
            EquipmentGenerationRequest request,
            string contentFingerprint,
            TraceAccumulator trace,
            string failureReason,
            RewardGenerationStatus status = RewardGenerationStatus.ImpossiblePolicy)
        {
            ...
            TraceAccumulator trace)
        {
            ...
            {
                AugmentGenerationCandidate candidate = request.Policy.AugmentCandidates[index];
                ...
                if (allowed)
                {
                    StableId provisionalId = RewardGenerationFingerprint.DeriveStableId(
                        "augment-provisional",
        ...
        private static string BuildEquipmentContentFingerprint(EquipmentGenerationRequest request)
        {
        ...
        {
            public WeightedEquipmentCandidate(EquipmentGenerationCandidate candidate, ulong weight)
            {
                Candidate = candidate;
                Weight = weight;
            }

            public EquipmentGenerationCandidate Candidate { get; }
            public ulong Weight { get; }
        }

        private sealed class WeightedQualityCandidate
        {
            public WeightedQualityCandidate(EquipmentQualityCandidate candidate, ulong weight)
            {
                Candidate = candidate;
                Weight = weight;
            }

            public EquipmentQualityCandidate Candidate { get; }
            public ulong Weight { get; }
        }
        ...
        private sealed class WeightedAugmentCandidate
        {
            public WeightedAugmentCandidate(
                AugmentGenerationCandidate candidate,
            ...
            public AugmentGenerationCandidate Candidate { get; }
