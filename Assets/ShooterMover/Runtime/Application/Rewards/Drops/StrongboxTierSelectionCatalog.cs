...
namespace ShooterMover.Application.Rewards.Drops
{
    ...
    public static class StrongboxTierSelectionCatalog
    {
        ...
        public static StrongboxTierSelectionProfile Get(StableId profileStableId) { StrongboxTierSelectionProfile profile; if (!TryResolve(profileStableId, out profile)) throw new KeyNotFoundException("Unknown strongbox tier-selection profile " + profileStableId + "."); return profile; }
        ...
        {
            IReadOnlyList<StrongboxTierWeight> weights = Get(profileStableId).Evaluate(activeContextIds); ulong total = 0UL;
        ...
        private static StrongboxTierSelectionProfile Profile(StableId id, params ulong[] weights)
        {
            ...
            var baseWeights = new List<StrongboxTierWeight>(11); var modifiers = new List<StrongboxTierContextModifier>();
            for (int index = 0; index < weights.Length; index++)
            {
                StableId tierId = StrongboxCatalog.GetByNumber(index + 1).TierStableId; baseWeights.Add(new StrongboxTierWeight(tierId, weights[index])); int tierNumber = index + 1;
                if (tierNumber >= 5) modifiers.Add(new StrongboxTierContextModifier(StableId.Parse("difficulty.hard"), tierId, 1200));
                if (tierNumber >= 7) modifiers.Add(new StrongboxTierContextModifier(StableId.Parse("difficulty.nightmare"), tierId, 1500));
                if (tierNumber >= 8) modifiers.Add(new StrongboxTierContextModifier(StableId.Parse("game-mode.raid"), tierId, 1500));
                if (tierNumber >= 6) modifiers.Add(new StrongboxTierContextModifier(StableId.Parse("event.strongbox-tier-boost"), tierId, 2000));
            }
            return new StrongboxTierSelectionProfile(id, baseWeights, modifiers);
        }
    }
}
