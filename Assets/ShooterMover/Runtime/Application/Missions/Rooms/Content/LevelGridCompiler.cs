...
namespace ShooterMover.Application.Missions.Rooms.Content
{
    ...
    public sealed class LevelGridSourcePackage
    {
        ...
        public LevelGridSourcePackage(IDictionary<string, string> documents)
        {
    ...
    public sealed class LevelGridCompileIssue
    {
        public LevelGridCompileIssue(string code, string path, string message)
        {
    ...
    public sealed class LevelGridCompileResult
    {
        private readonly IReadOnlyList<LevelGridCompileIssue> issues;

        internal LevelGridCompileResult(
            string levelId,
            RoomContentJsonPackage package,
            IReadOnlyList<LevelGridCompileIssue> issues)
        {
            LevelId = levelId;
            Package = package;
            this.issues = issues ?? Array.Empty<LevelGridCompileIssue>();
        }

        public string LevelId { get; }
        public RoomContentJsonPackage Package { get; }
        public IReadOnlyList<LevelGridCompileIssue> Issues { get { return issues; } }
    ...
    public static partial class LevelGridCompiler
    {
        ...
        public static LevelGridCompileResult Compile(LevelGridSourcePackage source)
        {
        ...
        private static LevelGridCompileResult Failure(
            string levelId,
            string code,
            string path,
            string message)
        {
            return new LevelGridCompileResult(
                levelId,
                null,
                new[] { new LevelGridCompileIssue(code, path, message) });
        }
        ...
        {
            private readonly LevelGridSourcePackage source;
            ...
            public Compiler(LevelGridSourcePackage source)
            {
                this.source = source;
            }

            public LevelGridCompileResult Compile()
            {
                ...
                RoomContentJsonPackage package = BuildV1Package(
                    levelId,
                    startRoomId,
                    finalRoomId,
                    finalDoorId);
                return new LevelGridCompileResult(
                    levelId,
                    package,
                    Array.Empty<LevelGridCompileIssue>());
            }
