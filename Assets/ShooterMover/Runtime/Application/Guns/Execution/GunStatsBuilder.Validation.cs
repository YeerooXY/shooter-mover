...
namespace ShooterMover.Application.Guns.Execution
{
    ...
    {
        private static GunProfileResolution Reject(
            GunProfileResolutionStatus status,
            string code)
        {
            return GunProfileResolution.Reject(status, code);
        }

        private static bool Validate(GunDefinitionData definition, out string code)
        {
            if (definition.BurstCount != 1 || definition.HealingPerSecond > Epsilon)
            {
                code = "gun-effect-unsupported:" + definition.DefinitionId;
                return false;
            }

            if (!IsPositive(definition.FireRate)
                || definition.ProjectilesPerTrigger < 1
                || definition.ProjectilesPerTrigger > GunLiveFiringProfile.MaximumEffectsPerFire
            ...
            {
                code = "gun-tuning-invalid:" + definition.DefinitionId;
                return false;
            }
            ...
            if (effectFamilies > 1)
            {
                code = "gun-effect-unsupported-combination:" + definition.DefinitionId;
                return false;
            }
            ...
            {
                code = "gun-tuning-invalid-explosion:" + definition.DefinitionId;
                return false;
            }
            ...
            {
                code = "gun-tuning-invalid-chain:" + definition.DefinitionId;
                return false;
            }
            ...
            {
                code = "gun-effect-unsupported-damage-over-time-without-pool:"
            ...
            if (!explosive
                ...
                && !IsPositive(definition.DamagePerProjectile))
            {
                code = "gun-tuning-invalid-direct:" + definition.DefinitionId;
                return false;
            }
