...
namespace ShooterMover.Application.Guns.Execution
{
    ...
    /// move projectiles, decide impacts, or alter the retained GunEffectBatch authority.
    /// </summary>
        ...
        public ProjectileEmissionResult Emit(ProjectileImpactDecision decision)
        {
            ...
            if (canonicalRocket
                ...
                && (decision.ExplosionTriggerReasons
                    & GunExplosionTriggerReason.EnemyImpact) == 0)
            {
            ...
            if (decision.ExplosionTriggerReasons != GunExplosionTriggerReason.None)
            {
