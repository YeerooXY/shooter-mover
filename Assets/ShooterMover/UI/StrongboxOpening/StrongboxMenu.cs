...
namespace ShooterMover.UI.StrongboxOpening
{
            ...
            GunCatalog gunCatalog)
        {
            for (int index = 0; index < payload.EquipmentInstances.Count; index++)
            {
                ...
                string artReferenceId =
                    GunArtReferenceResolver.TryResolve(
                        instance,
                        equipmentCatalog,
                        gunCatalog,
                        out artProjection,
    ...
    public sealed class StrongboxMenu : MonoBehaviour
    {
            ...
            StrongboxOpeningPresentationResult presentation)
        {
            ...
            for (int index = 0; index < presentation.Items.Count; index++)
            {
                ...
                if (!item.HasGunArtReference
                    || preloadedGunArt.ContainsKey(
                        item.GunArtReferenceId))
                {
                    continue;
                }
        ...
        private void DrawGunArt(StrongboxRewardRevealItem item)
        {
            if (item == null || !item.HasGunArtReference)
            {
                return;
            }

            GunArtSpriteResolution resolution;
            if (!preloadedGunArt.TryGetValue(
                item.GunArtReferenceId,
                out resolution))
            {
                resolution = GunArt.Preload(
                    item.GunArtReferenceId);
                preloadedGunArt[item.GunArtReferenceId] = resolution;
            }
