...
namespace ShooterMover.UI.StrongboxOpening
{
    ...
    public sealed class StrongboxOpeningPresentationView : MonoBehaviour
    {
        private StrongboxRewardCardsView rewardCards;
        private GUIStyle stageLabelStyle;

        public StrongboxOpeningSceneSession Session { get; private set; }

        public void Bind(
            StrongboxOpeningSceneSession immutableSession,
            StrongboxRewardCardsView rewardCardsView)
        {
        ...
        private void DrawAnimatedBox(Rect area)
        {
            ...
            if ((int)Session.Stage
                >= (int)StrongboxRevealStage.RewardReveal)
            {
                openingOffset = 28f;
            }

            float pulse =
                Session.Stage == StrongboxRevealStage.OpeningAnimation
                    ? 1f
        ...
        private string ResolveStageLabel(float progress)
        {
            ...
            {
                case StrongboxRevealStage.BoxClosed:
                    return "CLOSED";
                case StrongboxRevealStage.OpeningAnimation:
                    return "OPENING "
                        + Mathf.RoundToInt(progress * 100f)
                            .ToString(CultureInfo.InvariantCulture)
                        + "%";
                case StrongboxRevealStage.RewardReveal:
                    return "REVEAL";
                default:
                    return "COMPLETE";
            }
