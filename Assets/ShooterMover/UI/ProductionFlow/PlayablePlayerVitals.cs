...
namespace ShooterMover.UI.ProductionFlow
{
    ...
    public sealed class PlayablePlayerVitalsInstaller : MonoBehaviour
    {
        ...
        private static void TryInstall(Scene scene)
        {
            ...
            PlayableLevelController controller = FindController(scene);
            if (controller == null)
            {
            ...
            if (controller.GetComponent<PlayablePlayerVitalsInstaller>() == null)
            {
                controller.gameObject.AddComponent<PlayablePlayerVitalsInstaller>();
            }
        }

        private static PlayableLevelController FindController(Scene scene)
        {
            GameObject[] roots = scene.GetRootGameObjects();
            for (int index = 0; index < roots.Length; index++)
            {
                PlayableLevelController controller = roots[index]
                    .GetComponentInChildren<PlayableLevelController>(true);
                if (controller != null)
                {
                    return controller;
                }
