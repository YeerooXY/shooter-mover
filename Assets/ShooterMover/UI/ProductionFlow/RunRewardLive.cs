...
{
    internal sealed class RunRewardLive
    {
        private readonly PendingTerminalDropAdmissionState pending;
        private readonly PendingAdmissionViewConsumer projection;

        private RunRewardLive(
            RunSessionState authority,
            RunSessionAggregate run,
            PendingTerminalDropAdmissionState pending,
        ...
        public static RunRewardLive Create(
            ...
            StableId proofPlacementId)
        {
            ...
            var runContexts = new RunSessionTerminalDropContextResolver(
                authority,
            ...
            Func<EnemyTerminalDropFactConsumer> consumerFactory = delegate
            {
                ...
                TerminalDropBindingSetup binding =
                    TerminalDropBindingSetup.Create(
                        enemyCatalog,
                        enemySourceContexts,
                        propCatalog,
                        propSourceContexts,
        ...
        IPendingTerminalDropAdmissionConsumer
    {
        ...
        public PendingRunRewardView Export(
            PendingTerminalDropAdmissionState authority)
        {
            ...
            foreach (StableId operation in operations.OrderBy(value => value))
            {
                ...
                {
                    GeneratedTerminalDropReward reward = result.GeneratedRewards[index];
        ...
        IEnemyDropFactConsumer
    {
        ...
        private readonly PendingTerminalDropAdmissionState pending;
        private readonly PendingAdmissionViewConsumer projection;
        private readonly Func<EnemyTerminalDropFactConsumer> consumerFactory;
        private readonly StableId runId;
        private readonly StableId proofRoomId;
        private readonly StableId proofPlacementId;
        private readonly Dictionary<StableId, EnemyTerminalDropFactConsumer>
            committedByDeathEvent =
                new Dictionary<StableId, EnemyTerminalDropFactConsumer>();

        public TransactionalRunRewardEnemyConsumer(
            RunSessionAggregate run,
            PendingTerminalDropAdmissionState pending,
            PendingAdmissionViewConsumer projection,
            Func<EnemyTerminalDropFactConsumer> consumerFactory,
            StableId runId,
            StableId proofRoomId,
            StableId proofPlacementId)
        {
        ...
        public void Consume(EnemyDeathFact fact)
        {
            ...
            lock (gate)
            {
                EnemyTerminalDropFactConsumer committed;
                ...
                EnemyTerminalDropFactConsumer attempt = null;
                try
                {
            ...
            GeneratedTerminalDropResult result)
        {
            ...
            for (int index = 0; index < result.GeneratedRewards.Count; index++)
            {
                GeneratedTerminalDropReward reward =
        ...
        ITerminalRewardOverrideResolver
    {
            ...
            StableId placementId)
        {
            ...
            RewardSourceProfile profile = RewardSourceProfile.Create(
