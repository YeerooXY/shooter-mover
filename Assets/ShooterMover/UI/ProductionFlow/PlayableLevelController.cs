...
namespace ShooterMover.UI.ProductionFlow
{
    ...
    public sealed class PlayableLevelController : MonoBehaviour
    {
        [SerializeField] private JsonRoomLiveBootstrap2D roomBootstrap;
        [SerializeField] private RoomLiveSetup2D roomRuntime;
        ...
        private static void TryStartScene(Scene scene)
        {
            ...
            PlayableLevelController controller = FindInScene(scene);
            if (controller == null)
            {
        ...
        private static PlayableLevelController FindInScene(Scene scene)
        {
            GameObject[] roots = scene.GetRootGameObjects();
            for (int index = 0; index < roots.Length; index++)
            {
                PlayableLevelController value = roots[index]
