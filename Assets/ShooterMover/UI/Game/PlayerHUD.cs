...
{
    /// <summary>
    /// Neutral gameplay-facing player damage boundary. The retained PlayerActorState
    /// remains the sole owner of health, replay admission and the alive/dead lifecycle.
    /// </summary>
    public interface IPlayablePlayerDamageReceiver : IDamageReceiver
    {
        ...
        bool IsDefeated { get; }
        event Action<PlayablePlayerDefeatedFact> Defeated;
    ...
    /// </summary>
    public static class PlayablePlayerDamageCommandFactory
    {
    ...
    /// </summary>
    public interface IPlayablePlayerHubReturnRequest
    {
        bool TryReturnToHub(
            PlayerMarker player,
            out string rejectionCode);
    ...
    /// </summary>
    public static class PlayablePlayerHubReturnStateGuard
    {
    ...
    /// </summary>
    public sealed class PlayablePlayerHubReturnRequest :
        IPlayablePlayerHubReturnRequest
    {
            ...
            out string rejectionCode)
        {
            ...
            if (!PlayablePlayerHubReturnStateGuard.TryValidate(
                    player,
    ...
    public sealed class PlayablePlayerDefeatedFact
    {
        public PlayablePlayerDefeatedFact(
    ...
    public sealed class PlayerHUDInstaller : MonoBehaviour
    {
        ...
        private static void TryInstall(Scene scene)
        {
            ...
            LevelGame controller = FindController(scene);
            if (controller == null)
            {
            ...
            if (controller.GetComponent<PlayerHUDInstaller>() == null)
            {
                controller.gameObject.AddComponent<PlayerHUDInstaller>();
            }
        }

        private static LevelGame FindController(Scene scene)
        {
            GameObject[] roots = scene.GetRootGameObjects();
            for (int index = 0; index < roots.Length; index++)
            {
                LevelGame controller = roots[index]
        ...
        {
            PlayerMarker marker = GetComponentInChildren<
                PlayerMarker>(true);
            if (marker == null)
            {
                return;
            ...
            PlayerHUD vitals = marker.GetComponent<
                PlayerHUD>();
            if (vitals == null)
            {
                vitals = marker.gameObject.AddComponent<PlayerHUD>();
            }
            if (!vitals.IsBound)
            {
    ...
    /// <summary>
    /// Scene-local presentation and flow adapter around PlayerActorState. Persistent
        ...
        IPlayablePlayerDamageReceiver
    {
        ...
        private PlayerActorState authority;
        private IPlayablePlayerHubReturnRequest hubReturnRequest;
            ...
            IPlayablePlayerHubReturnRequest configuredHubReturnRequest)
        {
            ...
            StableId participantStableId = StableId.Create(
                "participant",
                "playable-level-" + runEntryToken);
            PlayerActorCreationResult creation = PlayerActorState.TryCreate(
        ...
        private void AcceptDefeat(GameplayEntityDeathFact deathFact)
        {
            ...
            var fact = new PlayablePlayerDefeatedFact(
        ...
        private void PublishDefeated(PlayablePlayerDefeatedFact fact)
        {
            ...
            {
                var observer =
                    (Action<PlayablePlayerDefeatedFact>)observers[index];
                try
                {
                    observer(fact);
                }
