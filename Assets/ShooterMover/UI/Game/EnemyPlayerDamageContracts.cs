...
namespace ShooterMover.UI.Game
{
    ...
    /// <summary>
    /// Owns exact EnemyAttack subscriptions. Reconciliation replaces the complete set, so
    /// room rebuilds cannot accumulate duplicate hit delivery.
    /// </summary>
    public sealed class EnemyHitSubscriptionSet
    {
        private readonly Dictionary<EnemyAttack, Action<EnemyHit>> subscriptions =
            new Dictionary<EnemyAttack, Action<EnemyHit>>();

        public int Count { get { return subscriptions.Count; } }

        public bool Contains(EnemyAttack publisher)
        {
            return publisher != null && subscriptions.ContainsKey(publisher);
        }

        public void Replace(
            IReadOnlyList<EnemyAttack> publishers,
            Action<EnemyAttack, EnemyHit> receiver)
        {
            if (publishers == null) throw new ArgumentNullException(nameof(publishers));
            if (receiver == null) throw new ArgumentNullException(nameof(receiver));

            Clear();
            var seen = new HashSet<EnemyAttack>();
            try
            {
                for (int index = 0; index < publishers.Count; index++)
                {
                    ...
                    EnemyAttack capturedPublisher = publisher;
        ...
        {
            foreach (KeyValuePair<EnemyAttack, Action<EnemyHit>> pair
                in subscriptions)
            {
                EnemyAttack publisher = pair.Key;
                if (publisher == null
