    ...
    public sealed class AtomicSaveAndCompensationTests
    {
        ...
        public void TemporaryInterruptionAndCorruptActiveRecoverLastKnownGood()
        {
            ...
            PlayerAccountStoreResult interrupted = store.Save(
                second.WithAccountComponent(new SavePartSnapshot(
        ...
        private static ISavePart Adapter(
            string suffix,
            int order,
            TestState authority)
        {
            var definition = new SavePartDefinition(
                Id("test-component." + suffix),
                1,
                "test-snapshot-v1",
                true,
                order);
            return new SnapshotSavePart<TestSnapshot>(
                definition,
        ...
        private sealed class TestState
        {
            ...
            public SavePartApplyResult Apply(TestSnapshot snapshot)
            {
        ...
        private sealed class TestSnapshotCodec :
            ExplicitSavePartCodec<TestSnapshot>
        {
                ...
                TestSnapshot snapshot)
            {
                return snapshot == null
                    ? SavePartValidationResult.Reject("test-snapshot-null")
