#if UNITY_EDITOR
using System;
using System.Reflection;
using NUnit.Framework;
using ShooterMover.Application.Rewards.Drops;
using ShooterMover.Domain.Enemies.Catalog;
using ShooterMover.EnemyRuntimeComposition;
using ShooterMover.TerminalDropBinding;

namespace ShooterMover.Tests.EditMode.TerminalDropBinding
{
    public sealed class TerminalDropPendingPublicationPolicyTests
    {
        private sealed class RejectOncePending :
            IGeneratedTerminalDropPendingAdmission
        {
            private readonly PendingTerminalDropAdmissionState inner;
            private bool rejected;

            public RejectOncePending(PendingTerminalDropAdmissionState inner)
            {
                this.inner = inner;
            }

            public PendingTerminalDropAdmissionResult Admit(
                GeneratedTerminalDropResult result)
            {
                if (!rejected)
                {
                    rejected = true;
                    return PendingTerminalDropAdmissionResult.Rejected(
                        "pending-publication-rejected-for-test");
                }
                return inner.Admit(result);
            }
        }

        [Test]
        public void Validate_RejectsRejectedGenerationBatch()
        {
            var batch = new TerminalPersonalRewardBatch(
                TerminalPersonalRewardBatchStatus.Rejected,
                null,
                Array.Empty<GeneratedTerminalDropResult>(),
                "generation-rejected-for-test");

            InvalidOperationException error =
                Assert.Throws<InvalidOperationException>(() =>
                    TerminalDropPendingPublicationPolicy.Validate(
                        batch,
                        Array.Empty<PendingTerminalDropAdmissionResult>()));

            StringAssert.Contains("generation-rejected-for-test", error.Message);
        }

        [Test]
        public void Validate_AcceptsNoEligibleParticipantsWithoutAdmissions()
        {
            var batch = new TerminalPersonalRewardBatch(
                TerminalPersonalRewardBatchStatus.NoEligibleParticipants,
                null,
                Array.Empty<GeneratedTerminalDropResult>(),
                string.Empty);

            Assert.DoesNotThrow(() =>
                TerminalDropPendingPublicationPolicy.Validate(
                    batch,
                    Array.Empty<PendingTerminalDropAdmissionResult>()));
        }

        [Test]
        public void StrictConsumer_RejectedPendingPublicationThrowsThenRetrySucceeds()
        {
            TerminalDropGenerationState authority;
            EnemyDeathFact death;
            CreateExistingEnemyPipeline(out authority, out death);
            var durable = new PendingTerminalDropAdmissionState();
            var consumer = new EnemyTerminalDropFactConsumer(
                authority,
                new RejectOncePending(durable),
                null,
                true);

            InvalidOperationException error =
                Assert.Throws<InvalidOperationException>(() =>
                    consumer.Consume(death));

            StringAssert.Contains("not admitted", error.Message);
            Assert.That(durable.PendingBatchCount, Is.EqualTo(0));

            Assert.DoesNotThrow(() => consumer.Consume(death));
            Assert.That(durable.PendingBatchCount, Is.EqualTo(1));
            Assert.That(consumer.LastAdmission.Status,
                Is.EqualTo(PendingTerminalDropAdmissionStatus.Accepted));
        }

        [Test]
        public void RollbackAccepted_RemovesOnlyExactNewAdmissionAndIsIdempotent()
        {
            GeneratedTerminalDropResult generated = GenerateExistingResult();
            var pending = new PendingTerminalDropAdmissionState();
            PendingTerminalDropAdmissionResult accepted =
                pending.Admit(generated);

            Assert.That(accepted.Status,
                Is.EqualTo(PendingTerminalDropAdmissionStatus.Accepted));
            Assert.That(pending.PendingBatchCount, Is.EqualTo(1));

            string diagnostic;
            Assert.That(
                pending.TryRollbackAccepted(accepted, out diagnostic),
                Is.True,
                diagnostic);
            Assert.That(pending.PendingBatchCount, Is.EqualTo(0));

            Assert.That(
                pending.TryRollbackAccepted(accepted, out diagnostic),
                Is.True,
                diagnostic);
            Assert.That(pending.PendingBatchCount, Is.EqualTo(0));
        }

        [Test]
        public void RollbackAccepted_RejectsExactReplayReceipt()
        {
            GeneratedTerminalDropResult generated = GenerateExistingResult();
            var pending = new PendingTerminalDropAdmissionState();
            PendingTerminalDropAdmissionResult accepted =
                pending.Admit(generated);
            PendingTerminalDropAdmissionResult replay =
                pending.Admit(generated);

            Assert.That(accepted.Status,
                Is.EqualTo(PendingTerminalDropAdmissionStatus.Accepted));
            Assert.That(replay.Status,
                Is.EqualTo(PendingTerminalDropAdmissionStatus.ExactReplay));

            string diagnostic;
            Assert.That(
                pending.TryRollbackAccepted(replay, out diagnostic),
                Is.False);
            StringAssert.Contains("receipt-invalid", diagnostic);
            Assert.That(pending.PendingBatchCount, Is.EqualTo(1));
        }

        private static GeneratedTerminalDropResult GenerateExistingResult()
        {
            Type fixtureType = typeof(TerminalDropReviewBlockerTests);
            MethodInfo factory = RequireMethod(fixtureType, "PipelineAuthority");
            Type factType = fixtureType.GetNestedType(
                "PipelineFact",
                BindingFlags.NonPublic);
            Assert.That(factType, Is.Not.Null);

            var authority = (TerminalDropGenerationState)factory.Invoke(
                null,
                new object[] { null, null, null, null });
            object fact = Activator.CreateInstance(
                factType,
                BindingFlags.Instance | BindingFlags.Public
                    | BindingFlags.NonPublic,
                null,
                new object[] { "transaction-policy" },
                null);
            GeneratedTerminalDropResult result = authority.Generate(fact);
            Assert.That(result.IsAccepted, Is.True);
            return result;
        }

        private static void CreateExistingEnemyPipeline(
            out TerminalDropGenerationState authority,
            out EnemyDeathFact death)
        {
            Type fixtureType = typeof(TerminalDropReviewBlockerTests);
            MethodInfo definitionFactory = RequireMethod(
                fixtureType,
                "EnemyDefinition");
            MethodInfo authorityFactory = RequireMethod(
                fixtureType,
                "EnemyAuthority");
            MethodInfo deathFactory = RequireMethod(
                fixtureType,
                "EnemyDeath");

            var definition = (EnemyDefinition)definitionFactory.Invoke(
                null,
                Array.Empty<object>());
            Type contextType = fixtureType.GetNestedType(
                "EnemyContextResolver",
                BindingFlags.NonPublic);
            Type generatorType = fixtureType.GetNestedType(
                "CountingGenerator",
                BindingFlags.NonPublic);
            Assert.That(contextType, Is.Not.Null);
            Assert.That(generatorType, Is.Not.Null);

            object contexts = Activator.CreateInstance(
                contextType,
                true);
            object generator = Activator.CreateInstance(
                generatorType,
                true);
            authority = (TerminalDropGenerationState)authorityFactory.Invoke(
                null,
                new[] { (object)definition, contexts, generator });
            death = (EnemyDeathFact)deathFactory.Invoke(
                null,
                new object[] { definition });
            Assert.That(authority, Is.Not.Null);
            Assert.That(death, Is.Not.Null);
        }

        private static MethodInfo RequireMethod(Type type, string name)
        {
            MethodInfo method = type.GetMethod(
                name,
                BindingFlags.NonPublic | BindingFlags.Static);
            Assert.That(method, Is.Not.Null, name);
            return method;
        }
    }
}
#endif
