...
{
    public sealed partial class ShopScreenPresentationTests
    {
        [Test]
        public void SameRunProjectsSameDeterministicStockAndPrices()
        {
            ...
            ShopScreenView left = first.Session("run.shopui-deterministic").Open();
            ShopScreenView right = second.Session("run.shopui-deterministic").Open();
        ...
        public void PurchaseUsesAuthoritiesAndProjectsBalanceAndSoldState()
        {
            ...
            ShopScreenSession session = fixture.Session("run.shopui-applied");
            ShopScreenView before = session.Open();
            ShopScreenStockCard card = before.Stock[0];

            ShopScreenActionResult result = session.SubmitPurchase(
                Input("shop-screen-input.applied", card));

            Assert.That(result.Status, Is.EqualTo(ShopScreenActionStatus.PurchaseApplied));
        ...
        public void InsufficientFundsLeavesBalanceAndStockAvailable()
        {
            ...
            ShopScreenSession session = fixture.Session("run.shopui-insufficient");
            ShopScreenView before = session.Open();
            ShopScreenStockCard card = before.Stock[0];
            ...
            Assert.That(result.Status, Is.EqualTo(ShopScreenActionStatus.InsufficientFunds));
        ...
        {
            Fixture fixture = new Fixture(10000L, 2);
            ShopScreenSession session = fixture.Session("run.shopui-duplicate-input");
            ShopScreenStockCard card = session.Open().Stock[0];
            ShopScreenPurchaseInput input = Input(
                "shop-screen-input.exact-duplicate",
                card);

            ShopScreenActionResult applied = session.SubmitPurchase(input);
            long balanceAfter = fixture.Money.Balance;
            long holdingsAfter = fixture.Holdings.Sequence;
            ShopScreenActionResult replay = session.SubmitPurchase(input);

            Assert.That(applied.Status, Is.EqualTo(ShopScreenActionStatus.PurchaseApplied));
        ...
        {
            Fixture fixture = new Fixture(10000L, 2);
            ShopScreenSession session = fixture.Session("run.shopui-conflict");
            ShopScreenView projection = session.Open();
            StableId sharedInput = Id("shop-screen-input.conflict");

            session.SubmitPurchase(new ShopScreenPurchaseInput(
                sharedInput,
                projection.Stock[0].StockEntryStableId));
            ShopScreenActionResult conflict = session.SubmitPurchase(
                new ShopScreenPurchaseInput(
                    sharedInput,
                    projection.Stock[1].StockEntryStableId));

            Assert.That(conflict.Status,
                Is.EqualTo(ShopScreenActionStatus.ConflictingDuplicate));
        }
        ...
        public void PendingPurchaseRetriesSameInputExactlyOnce()
        {
            ...
            ShopScreenSession session = fixture.Session("run.shopui-pending");
            ShopScreenStockCard card = session.Open().Stock[0];
            ShopScreenPurchaseInput input = Input(
            ...
            ShopScreenActionResult pending = session.SubmitPurchase(input);
            ShopScreenStockCard pendingCard =
                pending.Projection.FindCard(card.StockEntryStableId);
            ShopScreenActionResult applied = session.SubmitPurchase(
                new ShopScreenPurchaseInput(
        ...
        {
            Fixture fixture = new Fixture(10000L, 2);
            ShopScreenSession session = fixture.Session("run.shopui-back");
            session.Open();

            ShopScreenRouteResult first = session.NavigateBack();
            ShopScreenRouteResult second = session.NavigateBack();

            Assert.That(first.Emitted, Is.True);
            Assert.That(first.Route, Is.EqualTo(ShopScreenRoute.Hub));
