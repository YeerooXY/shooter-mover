...
{
    public sealed class PlayerLiveSetupTests
    {
        [Test]
        public void Construction_CreatesExactlyOneAuthorityAndAcquiresInputOnce()
        {
            ...
            PlayerLiveConstructionResult result = fixture.Root.TryConstruct(
                fixture.Configuration,
                fixture.Attachments);

            Assert.That(result.Status, Is.EqualTo(PlayerLiveConstructionStatus.Constructed));
        ...
        public void DuplicateConstruction_IsRejectedWithoutSecondInputOwnership()
        {
            ...
            PlayerLiveConstructionResult duplicate = fixture.Root.TryConstruct(
                fixture.Configuration,
                fixture.Attachments);

            Assert.That(duplicate.Status,
                Is.EqualTo(PlayerLiveConstructionStatus.RejectedDuplicate));
            Assert.That(duplicate.RejectionCode,
                Is.EqualTo(PlayerLiveConstructionRejectionCode.AlreadyConstructed));
            Assert.That(fixture.Input.AcquireCount, Is.EqualTo(1));
        }
        ...
        public void Restart_AdvancesPlayerAndMovementGenerationTogether()
        {
            ...
            runtime.ApplyDamage(new PlayerDamageRequest(
                ...
                CombatChannel.Kinetic,
                1L));
            PlayerLiveRestartCommand command = new PlayerLiveRestartCommand(
                Id("operation", "restart-a"),
                Id("actor", "player-a"),
                1L,
                2L);

            PlayerLiveRestartResult result = runtime.Restart(command);
            PlayerLiveRestartResult replay = runtime.Restart(command);
            ...
            Assert.That(replay.Status, Is.EqualTo(PlayerLiveRestartStatus.Duplicate));
            Assert.That(fixture.Movement.RestartCount, Is.EqualTo(1));
        }
        ...
        public void StaleRestartGeneration_IsRejectedBeforeEitherAuthorityMoves()
        {
            ...
            Assert.That(result.Status,
                Is.EqualTo(PlayerLiveRestartStatus.RejectedByLifecycle));
            Assert.That(result.RejectionCode,
                Is.EqualTo(PlayerLiveRestartRejectionCode.StaleGeneration));
        ...
        public void SharedInputAdapter_CannotBeOwnedByTwoPlayerRuntimes()
        {
            ...
            PlayerLiveConstructionResult rejected = second.Root.TryConstruct(
                second.Configuration,
                second.Attachments);

            Assert.That(rejected.Status,
                Is.EqualTo(PlayerLiveConstructionStatus.RejectedOwnership));
            Assert.That(rejected.RejectionCode,
                Is.EqualTo(PlayerLiveConstructionRejectionCode.InputOwnershipUnavailable));
        ...
        private sealed class Fixture
        {
            ...
            {
                PlayerLiveConstructionResult result = Root.TryConstruct(
                    Configuration,
                    Attachments);
                Assert.That(result.Status, Is.EqualTo(PlayerLiveConstructionStatus.Constructed));
