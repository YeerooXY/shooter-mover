...
namespace ShooterMover.Tests.EditMode.PlayerRuntime
{
    public sealed class PlayerSetupTests
    {
        ...
        public void Construction_CreatesExactlyOneAuthorityAndAcquiresInputOnce()
        {
            ...
            PlayerSetupResult result = fixture.Root.TryConstruct(
                fixture.Configuration,
                fixture.Attachments);

            Assert.That(result.Status, Is.EqualTo(PlayerSetupStatus.Constructed));
        ...
        public void DuplicateConstruction_IsRejectedWithoutSecondInputOwnership()
        {
            ...
            PlayerSetupResult duplicate = fixture.Root.TryConstruct(
                fixture.Configuration,
                fixture.Attachments);

            Assert.That(duplicate.Status,
                Is.EqualTo(PlayerSetupStatus.RejectedDuplicate));
            Assert.That(duplicate.RejectionCode,
                Is.EqualTo(PlayerSetupRejectionCode.AlreadyConstructed));
            Assert.That(fixture.Input.AcquireCount, Is.EqualTo(1));
        }
        ...
        public void Restart_AdvancesPlayerAndMovementGenerationTogether()
        {
            ...
            runtime.ApplyDamage(new PlayerDamageRequest(
                ...
                CombatChannel.Kinetic,
                1L));
            PlayerRestartCommand command = new PlayerRestartCommand(
                Id("operation", "restart-a"),
                Id("actor", "player-a"),
                1L,
                2L);

            PlayerRestartResult result = runtime.Restart(command);
            PlayerRestartResult replay = runtime.Restart(command);
            ...
            Assert.That(replay.Status, Is.EqualTo(PlayerRestartStatus.Duplicate));
            Assert.That(fixture.Movement.RestartCount, Is.EqualTo(1));
        }
        ...
        public void StaleRestartGeneration_IsRejectedBeforeEitherAuthorityMoves()
        {
            ...
            Assert.That(result.Status,
                Is.EqualTo(PlayerRestartStatus.RejectedByLifecycle));
            Assert.That(result.RejectionCode,
                Is.EqualTo(PlayerRestartRejectionCode.StaleGeneration));
        ...
        public void SharedInputAdapter_CannotBeOwnedByTwoPlayerRuntimes()
        {
            ...
            PlayerSetupResult rejected = second.Root.TryConstruct(
                second.Configuration,
                second.Attachments);

            Assert.That(rejected.Status,
                Is.EqualTo(PlayerSetupStatus.RejectedOwnership));
            Assert.That(rejected.RejectionCode,
                Is.EqualTo(PlayerSetupRejectionCode.InputOwnershipUnavailable));
        ...
        private sealed class Fixture
        {
            ...
            {
                PlayerSetupResult result = Root.TryConstruct(
                    Configuration,
                    Attachments);
                Assert.That(result.Status, Is.EqualTo(PlayerSetupStatus.Constructed));
        ...
        private sealed class FakeInput : IPlayerControls
        {
            ...
            public int DisposeCount { get; private set; }
            public PlayerControlsOwnership CurrentOwnership { get; private set; }

            public bool TryAcquire(PlayerControlsOwnership ownership)
            {
                AcquireCount++;
            ...
            public bool Release(PlayerControlsOwnership ownership)
            {
        ...
        private sealed class FakeRunFlow : IPlayerRunFlow
        {
            public FakeRunFlow()
            {
