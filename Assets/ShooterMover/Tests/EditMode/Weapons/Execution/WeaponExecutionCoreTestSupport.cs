...
namespace ShooterMover.Tests.EditMode.Weapons.Execution
{
    public sealed partial class WeaponExecutionCoreTests
    {
        private static Harness HarnessFor(
            ...
            IWeaponBehaviorSelector selector = null,
            WeaponBehaviorRegistry registry = null,
            RecordingSink sink = null)
        {
            ...
            WeaponExecutionCore core = new WeaponExecutionCore(
                new TestOwnershipResolver(),
                equippedResolver,
                profiles,
                registry ?? WeaponBehaviorRegistry.CreateWithBuiltIns(),
                actualSink);
            return new Harness(core, actualSink);
        }

        private static WeaponCatalog Catalog(WeaponDefinitionData definition)
        {
        ...
        private static WeaponDefinitionData Definition(
            ...
            WeaponCatalogAvailability availability = WeaponCatalogAvailability.Live,
            double dotDps = 0d,
            int burstCount = 1)
        {
        ...
        private static WeaponFireCommand Command(
            ...
            WeaponVector2 origin = null)
        {
            return new WeaponFireCommand(
                new WeaponActorInstanceId(ActorStableId),
                ...
                tick,
                seed,
                origin ?? new WeaponVector2(2d, 3d),
                aim ?? new WeaponVector2(1d, 0d));
        }

        private sealed class Harness
        {
            public Harness(WeaponExecutionCore core, RecordingSink sink)
            {
                Core = core;
                Sink = sink;
            }

            public WeaponExecutionCore Core { get; }
        ...
        private sealed class RecordingSink : IWeaponEffectBatchSink
        {
            ...
            public List<WeaponEffectBatch> Batches { get; } = new List<WeaponEffectBatch>();
            public List<int> ValidatedCounts { get; } = new List<int>();

            public WeaponEffectBatchSinkResult TryAccept(WeaponEffectBatch batch)
            {
                int validated = 0;
                foreach (IWeaponEffectDescription effect in batch.Effects)
                {
                ...
                return Reject
                    ? WeaponEffectBatchSinkResult.Reject("test-sink-rejected")
                    : WeaponEffectBatchSinkResult.Accept();
            }
        }
        ...
        private sealed class ExactDefinitionSelector : IWeaponBehaviorSelector
        {
            ...
            private readonly WeaponBehaviorId behaviorId;
            private readonly DefaultWeaponBehaviorSelector fallback =
                new DefaultWeaponBehaviorSelector();

            public ExactDefinitionSelector(string id, WeaponBehaviorId behavior)
            {
                definitionId = id;
                behaviorId = behavior;
            }
        ...
        {
            public ThreeProjectileTestBehavior(WeaponBehaviorId id)
            {
                BehaviorId = id;
            }
            ...
            public WeaponBehaviorBuildResult Build(WeaponBehaviorContext context)
            {
                List<IWeaponEffectDescription> effects =
                    new List<IWeaponEffectDescription>();
