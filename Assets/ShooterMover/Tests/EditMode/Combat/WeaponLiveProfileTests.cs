...
{
    public sealed class WeaponLiveProfileTests
    {
        ...
        {
            WeaponLiveProfile profile = Build(new FixtureValues());
            ...
            Assert.That(WeaponLiveProfile.SupportedMountCount, Is.EqualTo(4));
            Assert.That(WeaponLiveProfile.NormalFireConsumesConsumable, Is.False);
            ...
            Assert.That(
                ...
                Is.EqualTo(WeaponLiveProfile.DeterministicIdentityNamespace));
            Assert.That(profile.DeterministicIdentity.Value, Has.Length.EqualTo(64));

            Type profileType = typeof(WeaponLiveProfile);
            ...
            Assert.DoesNotThrow(() => WeaponLiveProfileValidator.Validate(profile, KnownModules()));
        }
        ...
        public void ModuleInput_IsDefensivelyCopied()
        {
            ...
            WeaponLiveProfile profile = Build(
        ...
        {
            WeaponLiveProfile first = Build(new FixtureValues());
            WeaponLiveProfile second = WeaponLiveProfile.ParseCanonical(
        ...
        {
            WeaponLiveProfile baseline = Build(new FixtureValues());
            ...
            foreach (NamedMutation mutation in mutations)
            {
                ...
                WeaponLiveProfile changed = Build(values);
        ...
        public void Create_AcceptsNoneHeatAndChargeModesSeparately()
        {
            ...
            WeaponLiveProfile none = Build(noneValues);
            WeaponLiveProfile heat = Build(new FixtureValues());
            WeaponLiveProfile charge = Build(ChargeValues());
        ...
        {
            WeaponLiveProfile configured = Build(new FixtureValues());
            WeaponLiveProfile absent = Build(new FixtureValues
            {
        ...
        public void Create_RejectsMissingUnknownNullAndDuplicateBehaviorModules()
        {
            ...
            Assert.Throws<ArgumentNullException>(
                () => WeaponLiveProfileValidator.Validate(Build(new FixtureValues()), null));
            Assert.Throws<ArgumentException>(
                () => Build(
                    new FixtureValues(),
        ...
        public void ParseCanonical_RejectsMalformedUnknownAndNonCanonicalInput()
        {
            ...
            Assert.Throws<FormatException>(
                () => WeaponLiveProfile.ParseCanonical(
                    string.Join("\n", lines.Take(lines.Length - 1).ToArray()),
                    KnownModules()));
            Assert.Throws<FormatException>(
                () => WeaponLiveProfile.ParseCanonical(canonical + "\n", KnownModules()));
            Assert.Throws<FormatException>(
                () => WeaponLiveProfile.ParseCanonical(
                    canonical.Replace("\n", "\r\n"),
                    KnownModules()));
        ...
        public void ModelContainsNoConsumableAmmunitionOrPackageSpecificSwitchSurface()
        {
            ...
            string[] memberNames = typeof(WeaponLiveProfile)
        ...
        {
            Assert.Throws<ArgumentNullException>(
                () => WeaponLiveProfileValidator.Validate(null, KnownModules()));
        }

        private static WeaponLiveProfile Build(
        ...
        {
            public int ProfileVersion = WeaponLiveProfile.CurrentProfileVersion;
