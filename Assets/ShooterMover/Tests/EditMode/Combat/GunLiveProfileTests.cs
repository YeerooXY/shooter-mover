...
{
    public sealed class GunLiveProfileTests
    {
        ...
        {
            GunLiveProfile profile = Build(new FixtureValues());

            Assert.That(profile.ProfileVersion, Is.EqualTo(1));
            Assert.That(profile.ProfileId, Is.EqualTo(StableId.Parse("gun-profile.synthetic")));
            Assert.That(GunLiveProfile.SupportedMountCount, Is.EqualTo(4));
            Assert.That(GunLiveProfile.NormalFireConsumesConsumable, Is.False);
            ...
            Assert.That(
                ...
                Is.EqualTo(GunLiveProfile.DeterministicIdentityNamespace));
            Assert.That(profile.DeterministicIdentity.Value, Has.Length.EqualTo(64));

            Type profileType = typeof(GunLiveProfile);
            ...
            Assert.DoesNotThrow(() => GunLiveProfileValidator.Validate(profile, KnownModules()));
        }
        ...
        {
            GunLiveProfile first = Build(new FixtureValues());
            GunLiveProfile second = GunLiveProfile.ParseCanonical(
        ...
        public void Fingerprint_IsSha256OfCanonicalUtf8Bytes()
        {
            ...
            Assert.That(
                ...
                Is.EqualTo("gun-runtime." + expected.Substring("sha256:".Length)));
        }

        [Test]
        public void EveryMeaningfulFieldChange_ChangesDeterministicIdentity()
        {
            GunLiveProfile baseline = Build(new FixtureValues());
            NamedMutation[] mutations =
            {
                Change("profile ID", values => values.ProfileId = StableId.Parse("gun-profile.changed")),
            ...
            foreach (NamedMutation mutation in mutations)
            {
                ...
                GunLiveProfile changed = Build(values);
        ...
        public void Create_AcceptsNoneHeatAndChargeModesSeparately()
        {
            ...
            GunLiveProfile none = Build(noneValues);
            GunLiveProfile heat = Build(new FixtureValues());
            GunLiveProfile charge = Build(ChargeValues());

            Assert.That(none.CycleMode, Is.EqualTo(GunCycleMode.None));
        ...
        public void Create_RejectsConflictingHeatAndChargeModes()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>(
                () => Build(new FixtureValues { CycleMode = (GunCycleMode)999 }));
        }
        ...
        {
            GunLiveProfile configured = Build(new FixtureValues());
            GunLiveProfile absent = Build(new FixtureValues
            {
                ProfileId = StableId.Parse("gun-profile.no-power-bank"),
                HasIndependentPowerBank = false,
                PowerBankCapacityUnits = 0d,
                EmpoweredCostUnits = 0d,
            });
        ...
        public void Create_RejectsMissingUnknownNullAndDuplicateBehaviorModules()
        {
            ...
            Assert.Throws<ArgumentNullException>(
                () => GunLiveProfileValidator.Validate(Build(new FixtureValues()), null));
            Assert.Throws<ArgumentException>(
                () => Build(
                    new FixtureValues(),
        ...
        public void ParseCanonical_RejectsMalformedUnknownAndNonCanonicalInput()
        {
            ...
            Assert.Throws<FormatException>(
                () => GunLiveProfile.ParseCanonical(
                    string.Join("\n", lines.Take(lines.Length - 1).ToArray()),
                    KnownModules()));
            Assert.Throws<FormatException>(
                () => GunLiveProfile.ParseCanonical(canonical + "\n", KnownModules()));
            Assert.Throws<FormatException>(
                () => GunLiveProfile.ParseCanonical(
                    canonical.Replace("\n", "\r\n"),
                    KnownModules()));
        ...
        public void ModelContainsNoConsumableAmmunitionOrPackageSpecificSwitchSurface()
        {
            ...
            string[] memberNames = typeof(GunLiveProfile)
        ...
        {
            Assert.Throws<ArgumentNullException>(
                () => GunLiveProfileValidator.Validate(null, KnownModules()));
        }

        private static GunLiveProfile Build(
        ...
        private static NamedMutation ChangeToChargeMode()
        {
            return Change(
                ...
                values =>
                {
                    values.CycleMode = GunCycleMode.Charge;
        ...
        {
            public int ProfileVersion = GunLiveProfile.CurrentProfileVersion;
            public StableId ProfileId = StableId.Parse("gun-profile.synthetic");
            ...
            public GunCycleMode CycleMode = GunCycleMode.Heat;
