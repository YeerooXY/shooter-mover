...
{
    public sealed class RoomContentJsonImporterTests
    {
        [Test]
        public void AnonymousEnemies_PreserveTypeAndIndependentLevelFacts()
        {
            RoomContentImportResult result = Import(
                EntryEnemies(
            ...
            RoomEnemyPlacementContent first = result.Bundle.Enemies[0];
            RoomEnemyPlacementContent second = result.Bundle.Enemies[1];
            ...
            AuthorableRoomDefinition room = result.Bundle.RuntimeDefinition.GetRoom(
        ...
        public void ReorderingDifferentAnonymousEnemies_PreservesGeneratedIdentityByFacts()
        {
            ...
            string levelTwo = "{\"object\":\"enemy.moving-droid\",\"level\":2,"
                + "\"position\":[4,4],\"rotation\":180}";
            RoomContentImportResult first = Import(
                EntryEnemies(levelOne + "," + levelTwo),
                EntryDecor(0, 0, 0, 0));
            RoomContentImportResult second = Import(
        ...
        public void TileFill_ExpandsInclusiveRectangleWithoutRepeatedCoordinates()
        {
            ...
            {
                RoomVisualPlacementContent visual = result.Bundle.Visuals[index];
        ...
        public void EncounterOwnsOptionalRoleRatherThanEnemyPlacement()
        {
            ...
            Assert.That(result.IsValid, Is.True, FirstIssue(result));
            RoomEnemyPlacementContent enemy = result.Bundle.Enemies[0];
            AuthorableRoomDefinition room = result.Bundle.RuntimeDefinition.GetRoom(
                StableId.Parse("room.test-entry"));
            RoomPlacedEntityDefinition runtimePlacement;
            Assert.That(
                room.TryGetPlacement(enemy.InstanceStableId, out runtimePlacement),
                Is.True);
            Assert.That(
                runtimePlacement.ClearRole,
                Is.EqualTo(RoomOccupantClearRole.OptionalEnemy));
        ...
        public void RoomBoundsDoorsAndSpawnKindTargets_CompileIntoRuntimeGraph()
        {
            ...
            Assert.That(result.IsValid, Is.True, FirstIssue(result));
            AuthorableRoomDefinition entry = result.Bundle.RuntimeDefinition.GetRoom(
                StableId.Parse("room.test-entry"));
            ...
            AuthorableRoomDefinition terminal = result.Bundle.RuntimeDefinition.GetRoom(
        ...
        private static RoomEnemyPlacementContent FindEnemy(
            RoomContentBundle bundle,
            int level)
        {
            for (int index = 0; index < bundle.Enemies.Count; index++)
            {
                RoomEnemyPlacementContent enemy = bundle.Enemies[index];
        ...
        private static string FirstIssue(RoomContentImportResult result)
        {
        ...
        private static RoomContentImportResult Import(
            string entryEnemies,
            string entryDecor,
            string entryEncounter = null)
        {
            ...
            return RoomContentJsonImporter.Import(
                new RoomContentJsonPackage(manifest, documents),
