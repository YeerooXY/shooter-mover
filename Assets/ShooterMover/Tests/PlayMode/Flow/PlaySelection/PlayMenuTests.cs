...
{
    public sealed class PlayMenuTests
    {
        [UnityTest]
        public IEnumerator SoloEmitsOneLevelSelectionRouteWithSamePayload()
        {
            ...
            PlayMenu controller = CreateController(
                payload,
                adapter);
        ...
        private static PlayMenu CreateController(
            PlayerRouteProfilePayload payload,
            IPlaySelectionRouteBridge adapter)
        {
            var gameObject = new GameObject("PlayMenuTests");
            PlayMenu controller =
