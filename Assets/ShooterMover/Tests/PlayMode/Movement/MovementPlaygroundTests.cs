...
namespace ShooterMover.Tests.PlayMode.Movement
{
    public sealed class MovementPlaygroundTests : InputTestFixture
    {
        ...
        public IEnumerator SceneLoad_ConstructsOneExplicitPlayerRoomCameraAndLifecycle()
        {
            ...
            Assert.That(body.GetComponents<PlayerControls>().Length, Is.EqualTo(1));
            Assert.That(body.GetComponents<MovementContact>().Length, Is.EqualTo(1));

            PlayerControls input = body.GetComponent<PlayerControls>();
            MovementContact contact = body.GetComponent<MovementContact>();
