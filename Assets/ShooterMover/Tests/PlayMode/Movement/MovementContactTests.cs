...
{
    public sealed class MovementContactTests
    {
        ...
        public void WallContact_UsesMT005AndAppliesReflectedOutcomeOnce()
        {
            ...
            FakeMovementContactState authority = CreateAuthority(tuning, -10d, 0d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
            Collider2D wall = CreateContactCollider(
                "MT-009 wall",
                MovementContactDescriptor.Wall());

            MovementContactProcessResult result = adapter.TryProcessContact(
                wall,
                Vector2.right,
                1d);

            Assert.That(result, Is.EqualTo(MovementContactProcessResult.WallReflected));
        ...
        public void LightEnemy_UsesCS004WeightAndShovesThrough()
        {
            ...
            Assert.That(result, Is.EqualTo(MovementContactProcessResult.EnemyResolved));
        ...
        {
            MovementThrusterTuningProfile tuning = BuildTuning();
            FakeMovementContactState authority = CreateAuthority(tuning, -8d, 3d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
        ...
        {
            MovementThrusterTuningProfile tuning = BuildTuning();
            FakeMovementContactState authority = CreateAuthority(tuning, -5d, 0d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
            ...
            Assert.That(result, Is.EqualTo(MovementContactProcessResult.UnknownContactIgnored));
        ...
        {
            MovementThrusterTuningProfile tuning = BuildTuning();
            FakeMovementContactState authority = CreateAuthority(tuning, -6d, -8d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
            Collider2D wall = CreateContactCollider(
                "MT-009 corner wall",
            ...
            MovementContactProcessResult first = adapter.TryProcessContact(
                wall,
                Vector2.right,
                5d);
            MovementContactProcessResult second = adapter.TryProcessContact(
                wall,
                Vector2.up,
                5d);

            Assert.That(first, Is.EqualTo(MovementContactProcessResult.WallReflected));
            Assert.That(second, Is.EqualTo(MovementContactProcessResult.WallReflected));
        ...
        {
            MovementThrusterTuningProfile tuning = BuildTuning();
            FakeMovementContactState authority = CreateAuthority(tuning, -9d, 0d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
            Collider2D wall = CreateContactCollider(
            ...
            MovementContactProcessResult duplicate = adapter.TryProcessContact(
                wall,
                new Vector2(2f, 0f),
                6d);

            Assert.That(first, Is.EqualTo(MovementContactProcessResult.WallReflected));
            Assert.That(duplicate, Is.EqualTo(MovementContactProcessResult.DuplicateCallbackIgnored));
            Assert.That(authority.WallApplyCount, Is.EqualTo(1));
        ...
        {
            MovementThrusterTuningProfile tuning = BuildTuning();
            FakeMovementContactState authority = CreateAuthority(tuning, -7d, -2d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
            ...
            MovementContactProcessResult first = adapter.TryProcessContact(
                ...
                7d);
            Vector2 afterFirst = body.linearVelocity;
            MovementContactProcessResult repeat = adapter.TryProcessContact(
                enemy,
                Vector2.up,
                7d);

            Assert.That(first, Is.EqualTo(MovementContactProcessResult.EnemyResolved));
            Assert.That(repeat, Is.EqualTo(MovementContactProcessResult.EnemyGraceIgnored));
        ...
        {
            MovementThrusterTuningProfile tuning = BuildTuning();
            FakeMovementContactState authority = CreateAuthority(tuning, -4d, 1d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
            Collider2D wall = CreateContactCollider(
            ...
            Assert.That(result, Is.EqualTo(MovementContactProcessResult.TunnelingSentinelIgnored));
            Assert.That(authority.SnapshotReadCount, Is.Zero);
            Assert.That(authority.TotalApplyCount, Is.Zero);
        ...
        {
            MovementThrusterTuningProfile tuning = BuildTuning();
            FakeMovementContactState authority = CreateAuthority(tuning, -4d, 0d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
            Collider2D wall = CreateContactCollider(
            ...
            Assert.That(result, Is.EqualTo(MovementContactProcessResult.InvalidContactIgnored));
            Assert.That(authority.TotalApplyCount, Is.Zero);
        }
        ...
        {
            MovementThrusterTuningProfile tuning = BuildTuning();
            FakeMovementContactState authority = CreateAuthority(tuning, -6d, 0d);
            Rigidbody2D body;
            MovementContact adapter = CreateAdapter(authority, out body);
            Collider2D wall = CreateContactCollider(
            ...
            Assert.That(
                adapter.TryProcessContact(wall, Vector2.right, 10d),
                Is.EqualTo(MovementContactProcessResult.DuplicateCallbackIgnored));
        ...
        public void AdapterSurface_ContainsOnlyUnity2DPhysicsTypes()
        {
            Type[] inspectedTypes =
            {
                ...
                typeof(IMovementContact),
                typeof(MovementContact),
                typeof(IMovementContactState),
            };
        ...
        private MovementContact CreateAdapter(
            FakeMovementContactState authority,
            out Rigidbody2D body)
        {
            ...
            MovementContact adapter = mover.AddComponent<MovementContact>();
            ...
            MovementContactDescriptor descriptor)
        {
            ...
            TestMovementContact contract =
                contactObject.AddComponent<TestMovementContact>();
        ...
        private static MovementContactDescriptor CreateEnemyDescriptor(
            StableId enemyId,
            ...
            CombatWeightClass targetWeight)
        {
            ...
            WeightMessage message = new WeightMessage(
                ...
                targetWeight,
                result);
            return MovementContactDescriptor.Enemy(enemyId, message);
        }

        private static FakeMovementContactState CreateAuthority(
            MovementThrusterTuningProfile tuning,
            double velocityX,
            double velocityY)
        {
            return new FakeMovementContactState(tuning, velocityX, velocityY);
        }
        ...
        private sealed class FakeMovementContactState : IMovementContactState
        {
            ...
            public FakeMovementContactState(
                MovementThrusterTuningProfile tuning,
                double velocityX,
                double velocityY)
            {
    ...
    public sealed class TestMovementContact : MonoBehaviour, IMovementContact
    {
        public MovementContactDescriptor Descriptor { get; set; }
