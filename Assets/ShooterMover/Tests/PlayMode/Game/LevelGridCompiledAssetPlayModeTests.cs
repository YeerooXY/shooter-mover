...
namespace ShooterMover.Tests.PlayMode.Game
{
    public sealed class LevelGridCompiledAssetPlayModeTests
    {
        private static readonly StableId StarterId =
            StableId.Parse("room.level-1-start");
        private static readonly StableId SingleId =
            StableId.Parse("room.level-1-middle");
        private static readonly StableId DoubleId =
            StableId.Parse("room.level-1-final");

        [UnityTest]
        public IEnumerator Level1Asset_LoadsAndImportsAfterRuntimeFrame()
        {
            yield return null;
        ...
        [UnityTest]
        public IEnumerator Level1Authority_TraversesGatesAndRestartsCleanly()
        {
            yield return null;
            ...
            var authority = new RoomFlowState(
                StableId.Parse("room-runtime-instance.level-1-playmode"),
                graph);
            ...
            var restarted = new RoomFlowState(
                StableId.Parse("room-runtime-instance.level-1-playmode-restart"),
                graph);
            Assert.That(
        ...
        {
            return StableId.Parse("operation.level-1-playmode-" + token);
        }
    }
}
