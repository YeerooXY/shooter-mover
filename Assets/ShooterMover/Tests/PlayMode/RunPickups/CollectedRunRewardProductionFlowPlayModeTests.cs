...
namespace ShooterMover.Tests.PlayMode.RunPickups
{
    public sealed class CollectedRunRewardProductionFlowPlayModeTests
    {
        ...
        public IEnumerator PhysicalCollectedRewardsPersistExactlyOnceAcrossResultsHubAndReload()
        {
            ...
            EquipmentInstance secondEquipment = EquipmentInstance.Create(
            ...
            RecordingRunJournal journal = new RecordingRunJournal(
                run,
                actor,
                participant);
            ...
            ExistingRunSessionPickupPortV1 pickupPort =
                new ExistingRunSessionPickupPortV1(journal);
            RunLocalPickupAuthorityV1 pickupAuthority =
                new RunLocalPickupAuthorityV1(pickupPort, positions);
            PendingTerminalDropPickupConsumerV1 pickupConsumer =
                new PendingTerminalDropPickupConsumerV1(pickupAuthority);
            ...
            PendingTerminalDropAdmissionAuthorityV1 admission =
                new PendingTerminalDropAdmissionAuthorityV1();
            Assert.That(
                pickupConsumer.Consume(admission.Admit(enemyDrop)).IsAccepted,
                Is.True);
            Assert.That(
                pickupConsumer.Consume(admission.Admit(propDrop)).IsAccepted,
                Is.True);
            Assert.That(presenter.Synchronize(room).Succeeded, Is.True);
            ...
            ExactEquipmentPayloadSource payloads =
            ...
            Assert.That(persistence.PersistPreparedCustody(prepared).Succeeded,
                Is.True);
            ProductionCollectedRunRewardTransferServiceV2 transfer =
            ...
            PlayerAccountSaveAuthorityV1 reloadAuthority =
                new PlayerAccountSaveAuthorityV1(durable);
            CharacterCompositionCoordinatorV1 reloadComposition =
                new CharacterCompositionCoordinatorV1(
                    reloadAuthority,
                    factory,
                    save);
            Assert.That(reloadComposition.Select(0).Succeeded, Is.True);
            ProductionCharacterRuntimeGraphV1 reloadedGraph =
                (ProductionCharacterRuntimeGraphV1)
            ...
            Assert.That(exactEquipment.EquipmentInstance.InstanceId,
                Is.EqualTo(equipmentInstance));
            Assert.That(exactEquipment.EquipmentInstance.Fingerprint,
            ...
            CollectedRunRewardTransferResultV1 secondRun =
                ApplyEquipmentOnlyRun(
                    "second",
            ...
            Assert.That(firstHeld.EquipmentInstance.DefinitionId,
                Is.EqualTo(secondHeld.EquipmentInstance.DefinitionId));
            Assert.That(firstHeld.EquipmentInstance.InstanceId,
                Is.Not.EqualTo(secondHeld.EquipmentInstance.InstanceId));

            StrongboxOpenCommandV1 openCommand =
            ...
            string label)
        {
            ...
            RunPickupPresentationEntryV1 entry =
                new RunPickupPresentationEntryV1();
            entry.Configure(
                kind,
                null,
                prefab,
            ...
            out CollectedRunRewardTransferReceiptAuthorityV1 receipts)
        {
            RewardApplicationServiceV1 rewardApplication =
                new RewardApplicationServiceV1(
            ...
            IReadOnlyList<GeneratedTerminalDropRewardV1> children)
        {
            ...
            RewardOperationRequestV1 request =
            ...
            CollectedRunRewardTransferReceiptAuthorityV1 receipts)
        {
            RunSessionCollectedRewardV1 reward =
                DirectEquipmentReward(suffix, run, equipment);
            EndRunSessionCommandV1 endCommand =
                new EndRunSessionCommandV1(
            ...
            ICollectedRunEquipmentPayloadSourceV2
        {
            ...
            public ExactEquipmentPayloadSource(
                params EquipmentInstance[] values)
            {
            ...
            IRunSessionCollectedRewardAuthorityV1
        {
            ...
            {
                return rewards
                    .OrderBy(item => item.CollectionOrder)
                    .ToList()
                    .AsReadOnly();
            }
        }
