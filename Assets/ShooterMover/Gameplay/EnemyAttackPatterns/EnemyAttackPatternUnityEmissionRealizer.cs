...
namespace ShooterMover.UnityAdapters.Enemies
{
    ...
    /// BoundedProjectile and HitResolver, and delegates all damage decisions to the
    /// shared Combat Hit Policy router.
    /// </summary>
    public sealed class EnemyAttackPatternUnityEmissionRealizer :
        IEnemyAttackPatternEmissionRealizer,
        IEnemyAttackPatternMeleeContactReporter,
        IDisposable
    {
        private sealed class SourceState
        {
            ...
            public EnemyAttackPatternUnitySourceBinding Binding { get; }
            public HitResolver HitAdapter { get; }
            public MeleeAttack MeleeContact { get; }
        }
        ...
        private readonly EnemyAttackPatternUnitySourceRegistry sourceRegistry;
        private readonly EnemyAttackPatternHitRouter hitRouter;
        ...
        public EnemyAttackPatternUnityEmissionRealizer(
            IEnemyAttackPatternRunTime runTime,
            EnemyAttackPatternUnitySourceRegistry sourceRegistry,
            EnemyAttackPatternHitRouter hitRouter,
            IEnemyAttackPatternDamageChannelMap channelMap = null)
        {
            ...
            this.channelMap = channelMap
                ?? new BuiltInEnemyAttackPatternDamageChannelMap();
        }
        ...
        public void AttachSource(StableId sourceEntityStableId)
        {
            ...
            var hitAdapter = new HitResolver(sourceEntityStableId);
            for (int targetIndex = 0;
                targetIndex < binding.Targets.Count;
                targetIndex++)
            {
                EnemyAttackPatternTargetBinding target =
                    binding.Targets[targetIndex];
                for (int colliderIndex = 0;
                    colliderIndex < target.Colliders.Count;
                    colliderIndex++)
                {
                    HitTargetRegistrationStatus status =
                        hitAdapter.RegisterTarget(
                            target.Colliders[colliderIndex],
                            target.TargetEntityStableId);
                    if (status != HitTargetRegistrationStatus.Registered
                        && status
                            != HitTargetRegistrationStatus.AlreadyRegistered)
                    {
                        throw new InvalidOperationException(
        ...
        public void Dispose()
        {
            ...
            var activeProjectiles = new List<BoundedProjectile>();
        ...
        {
            rejectionCode = string.Empty;
            EnemyAttackScheduledProjectile projectile = emission.Projectile;
            EnemyProjectilePayload payload =
                projectile == null ? null : projectile.Payload;
            BoundedProjectile prefab;
            if (payload == null
            ...
            if (!BoundedProjectile.IsValidSpeed(speed)
                || !BoundedProjectile.IsValidLifetime(lifetime)
        ...
        {
            rejectionCode = string.Empty;
            EnemyAttackScheduledMeleeStrike strike = emission.MeleeStrike;
            ...
            SourceState source)
        {
            ...
            channelMap.TryMap(
                emission.Execution.Descriptor.DamageChannelId,
                out channel);
            BoundedProjectile instance = UnityEngine.Object.Instantiate(
                prefab,
                origin,
                Quaternion.identity);
        ...
        private void HandleProjectileCompleted(BoundedProjectile projectile)
        {
            ...
            if (disposed
                || projectile.CompletionReason
                    == BoundedProjectileCompletionReason.Cancelled)
            {
                return;
            }

            EnemyAreaPayload area = state.Emission.Projectile.Payload.AreaPayload;
            if (area != null)
            {
            ...
            EnemyAttackPatternUnitySourceBinding binding)
        {
            ...
            {
                EnemyAttackPatternTargetBinding target = binding.Targets[index];
                result.Add(
                    target.TargetEntityStableId,
        ...
        private void ThrowIfDisposed()
        {
            if (disposed)
            {
                throw new ObjectDisposedException(
                    nameof(EnemyAttackPatternUnityEmissionRealizer));
            }
        }
    }
}
