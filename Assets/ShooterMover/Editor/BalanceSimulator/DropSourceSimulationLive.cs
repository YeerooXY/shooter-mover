...
namespace ShooterMover.EditorTools.BalanceSimulator
{
    ...
    /// </summary>
    public sealed class DropSourceSimulationLive
    {
        private sealed class ParticipantAccumulator
        {
            ...
            public ParticipantDropPacingState FinalPacingState;
        }

        private readonly RewardProfileResolver profileResolver;

        public DropSourceSimulationLive()
            : this(new RewardProfileResolver())
        {
        }
            ...
            DropSourceSimulationRequest request)
        {
            ...
            {
                RewardSimulationParticipantInput participant =
                ...
                sampleOrdinal++)
            {
                    ...
                    roomOrdinal++)
                {
                        ...
                        sourceOrdinal++)
                    {
                        ...
                        var contexts = new List<PersonalRewardRollContext>(
                    ...
                    participantIndex++)
                {
                    RewardSimulationParticipantInput participant =
                        request.Participants[participantIndex];
                    PersonalRewardRollContext completionContext =
                        BuildCompletionContext(
                            ...
                            runStableId,
                            sampleOrdinal);
                    PersonalRewardGenerationResult completion =
                    ...
                    ParticipantDropPacingState finalState;
            ...
            var participantReports =
                new List<RewardSimulationParticipantReport>(totals.Count);
            foreach (ParticipantAccumulator accumulator in totals.Values)
            {
                participantReports.Add(
                    new RewardSimulationParticipantReport(
            ...
            return new RewardSimulationReport(
                request.SourceProfileReferenceId,
        ...
        private static PersonalRewardRollContext BuildContext(
            DropSourceSimulationRequest request,
            ...
            int sourceOrdinal)
        {
            ...
            ulong personalSeed = request.Seed
                ^ RewardGenerationFingerprint.StableOrdinal(seedIdentity);
        ...
        private static PersonalRewardRollContext BuildCompletionContext(
            DropSourceSimulationRequest request,
            RewardProfileResolution resolution,
            RewardSimulationParticipantInput participant,
            StableId runStableId,
            int sampleOrdinal)
        {
            ...
            StableId placementStableId = StableId.Create(
                "simulation-placement",
                "sample-" + sampleOrdinal + "-run-completion");
            string fingerprint = RewardGenerationFingerprint.Compute(
            ...
            return new PersonalRewardRollContext(
                ...
                fingerprint,
                request.Seed
                    ^ RewardGenerationFingerprint.StableOrdinal(
                        terminalStableId)
                    ^ RewardGenerationFingerprint.StableOrdinal(
                        participant.ParticipantStableId),
                1);
            ...
            bool runMinimum)
        {
            ...
            for (int decisionIndex = 0;
                ...
                decisionIndex++)
            {
                PersonalRewardDecision decision = result.Decisions[decisionIndex];
                accumulator.RawStrongboxProbabilityMillionthsTotal +=
            ...
            for (int grantIndex = 0;
                ...
                grantIndex++)
            {
                RewardGrant grant = result.Grants[grantIndex];
                switch (grant.Kind)
                {
