...
{
    public static class GunLootCardEditorDrawer
    {
        ...
        {
            GunLootCardView card;
            string diagnostic;
            if (!GunLootCardView.TryCreate(
                    equipment,
                    equipmentCatalog,
                    gunCatalog,
                    out card,
                    out diagnostic))
            {
                EditorGUILayout.HelpBox(
                    "Gun card unavailable. Generation result was not projected: "
                    + diagnostic,
                    MessageType.Error);
                return false;
            }
            ...
            if (diagnosticsExpanded)
            {
                ...
                EditorGUILayout.LabelField(
                    "Runtime gun reference",
                    GunDefinitionId.FromRuntimeReference(
                        card.RuntimeGunReferenceId).Value);
                EditorGUILayout.LabelField(
                    "Gun definition",
                    card.GunDefinitionId);
                EditorGUILayout.LabelField(
                    "Raw quality ID",
