...
namespace ShooterMover.Editor.LevelDesign.Foundation
{
    public sealed partial class LevelGridEditorWindow : EditorWindow
    {
        private void ShowRootMenu()
        {
            ...
            for (int index = 0; index < roots.Length; index++)
            {
                ...
                string label = root.LevelIdText + " — "
                    + LevelGridEditorProblemLocator.BuildDiagnosticLocation(
                        root.transform);
                menu.AddItem(
        ...
        private void ValidateActiveRootDraft()
        {
            ...
            activeRoot.ValidateGridAuthoring(LevelGridValidationPurpose.Draft);
            projectionDirty = true;
        }
        ...
        private void OnUnitySelectionChanged()
        {
            ...
            if (selectedComponent != null
                && LevelGridEditorOperations.ResolveRoot(selectedComponent) == activeRoot)
            {
        ...
        private void EnsureProjection()
        {
            ...
            projection = LevelGridEditorView.Build(activeRoot);
        ...
        private bool HasProblemOtherThan(
            string authoredId,
            LevelGridProblemCode excludedCode)
        {
            if (activeRoot == null || string.IsNullOrEmpty(authoredId))
            {
                return false;
