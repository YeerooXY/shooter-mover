...
namespace ShooterMover.Editor.LevelDesign.Foundation
{
    ...
    public static partial class LevelGridPlayableExporter
    {
            ...
            string outputRoot)
        {
            ...
            if (metadata == null)
            {
                throw new InvalidOperationException(
                    "Add LevelGridPlayableMetadata to the level root before playable export.");
            }
            ...
            try
            {
                ...
                LevelGridPlayableProvenance.Write(root, stage);
        ...
        {
            LevelGridCompileResult compile = LevelGridAssetCompiler.CompileFolder(stage);
            ...
            LevelGridPlayableMetadata metadata)
        {
            ...
            var gridRooms = new List<LevelGridRoomRecord>(rooms.Length);
            ...
            var doorRecords = new List<LevelGridDoorRecord>(doors.Length);
            for (int index = 0; index < doors.Length; index++)
            {
                doorRecords.Add(doors[index].BuildRecord());
            }

            var connectionRecords = new List<LevelGridConnectionRecord>(links.Length);
            for (int index = 0; index < links.Length; index++)
            {
                connectionRecords.Add(links[index].BuildRecord());
            }

            LevelGridValidationResult result = LevelGridPlayableValidation.Validate(
                roomRecords,
                gridRooms,
                doorRecords,
                connectionRecords,
            ...
            {
                LevelGridProblem issue = result.Problems.Count == 0
                    ? null
                    : result.Problems[0];
                throw new InvalidOperationException(
                    issue == null
