...
{
    public sealed class EnemyReadinessWindow : EditorWindow
    {
        ...
        {
            GetWindow<EnemyReadinessWindow>("Enemy Readiness");
        }
        ...
        private static void Draw(ReadinessRow row)
        {
        ...
        private void RefreshCore()
        {
            ...
            EnemyCatalogImportResult enemyImport = EnemyCatalogJsonImporter.Import(
                enemyJson.text,
            ...
            RoomContentImportResult roomImport = roomContent.Import();
            ...
            out string failure)
        {
            ...
            RoomContentObjectCatalog objectCatalog =
                BuiltInRoomContentObjectCatalog.Create();
            for (int index = 0; index < bundle.Enemies.Count; index++)
            {
                RoomEnemyPlacementContent placement = bundle.Enemies[index];
                ...
                RoomContentObjectDefinition mapping;
        ...
        private static string JoinEnemyIssues(IReadOnlyList<EnemyCatalogIssue> issues)
        {
            if (issues == null || issues.Count == 0) return "no structured issue was returned.";
            var values = new List<string>();
            for (int index = 0; index < issues.Count; index++)
            {
                EnemyCatalogIssue issue = issues[index];
                values.Add(issue == null ? "<null issue>" : issue.ToString());
            }
            return string.Join(" | ", values);
        }

        private static string JoinRoomIssues(IReadOnlyList<RoomContentImportIssue> issues)
        {
            if (issues == null || issues.Count == 0) return "no structured issue was returned.";
            var values = new List<string>();
            for (int index = 0; index < issues.Count; index++)
            {
                RoomContentImportIssue issue = issues[index];
