...
This package provides reusable, definition-driven 2D breakables.

## Authority boundary

- `BreakableDamage` owns stable prop identity, health, confirmed-hit replay
...
- `BreakableFamilyDefinitionAsset` owns family defaults and arbitrary variants.
- Resolution is instance override, then selected variant, then family default.
- `PlacedObject` supplies the stable placed-instance identity and selected
  family/variant identity. Hierarchy names and sibling positions are never identifiers.
- `BreakableSetup` requires explicit collider and intact-renderer references,
...
- `BreakableLoot` submits the first destruction fact through SRC-001 once;
  it owns no reward value or application truth.

## Runtime behavior

`Breakable` keeps the existing four-argument `Configure` overload for compatible
...
`Stage1BreakableIntegration.Attach` remains as a bounded migration entry point for
the existing Stage 1 composition root. It first attempts normal definition-driven
configuration. A marker with no `PlacedObject` may then use the temporary legacy
path, which:

- discovers only `BreakableSetup` markers under the supplied presentation root;
