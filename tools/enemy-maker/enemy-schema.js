...
(function expose(root, factory) {
  ...
  function optionalText(errors, value, path) {
    if (value !== undefined && (typeof value !== "string" || value.trim() === "")) errors.push(`${path} must be a non-empty string when provided.`);
  }
  ...
  function validateEnemy(enemy, expectedId) {
    ...
    if (typeof enemy.id !== "string" || !ID.test(enemy.id)) errors.push("id must use lower-case letters, digits, and single hyphens.");
    ...
    if (enemy.speed !== undefined && (!finite(enemy.speed) || enemy.speed < 0)) errors.push("speed must be zero or greater when provided.");
    if (enemy.move !== undefined && !MOVES.has(enemy.move)) errors.push("move must be direct, wander, strafe, fly, or stationary.");
    if (!positive(enemy.scale)) errors.push("scale must be greater than zero.");
    if (enemy.type === "shooter" && (typeof enemy.gun !== "string" || enemy.gun.trim() === "")) errors.push("gun is required for a shooter.");
    if (enemy.type !== "shooter" && enemy.gun !== undefined) errors.push("gun is only valid for a shooter in the first iteration.");
    if (enemy.type !== "shooter" && !positive(enemy.damage)) errors.push("damage is required for non-shooter contact attacks.");
    if (enemy.type === "shooter" && enemy.damage !== undefined) errors.push("Shooter damage comes from the canonical gun and must not be copied into the enemy.");
  ...
  function validateLeveling(value) {
    ...
    if (!Array.isArray(value.colors) || value.colors.length < 2) {
      ...
      value.colors.forEach((stop, index) => {
        ...
        previous = stop.level;
        if (typeof stop.color !== "string" || !/^#[0-9a-fA-F]{6}$/.test(stop.color)) errors.push(`colors[${index}].color must be a six-digit hex color.`);
      });
  ...
  function levelColor(level, leveling) {
    ...
    for (let index = 1; index < stops.length; index += 1) {
      ...
      const a = parseHex(left.color), b = parseHex(right.color);
