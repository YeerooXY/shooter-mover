...
function currentEnemy() {
  if (jsonEditing) {
    try { return JSON.parse($("json").value); } catch (_) { return null; }
  }
  return compactEnemy();
}
...
function drawPreview() {
  ...
  if (!enemy || !leveling || schema.validateLeveling(leveling).length) return;

  const level = Number($("level").value);
  const stats = schema.resolvedStats(enemy, level, leveling);
  const cx = canvas.width / 2, cy = canvas.height / 2, unit = 18;
  const drawRing = (radius, color, dash) => {
    if (!(radius > 0)) return;
    ctx.save(); ctx.strokeStyle = color; ctx.lineWidth = 1.5; ctx.setLineDash(dash); ctx.beginPath(); ctx.arc(cx, cy, radius * unit, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
  };
  ...
  ctx.fill(); ctx.stroke();
  ctx.strokeStyle = "#dce4ef"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(bodyX, bodyY); ctx.lineTo(bodyX + 45, bodyY); ctx.stroke();
  ctx.fillStyle = "#dce4ef"; ctx.beginPath(); ctx.arc(cx, cy, 3, 0, Math.PI * 2); ctx.fill();
...
async function loadEnemy(id) {
  ...
  fillEnemy(result.enemy);
}

async function loadAll() {
  const [status, list, levelData] = await Promise.all([request("/api/status"), request("/api/enemies"), request("/api/leveling")]);
  mutationToken = status.token;
  $("contentPath").textContent = status.content;
  enemies = list.enemies;
  leveling = levelData.leveling;
  renderLeveling();
  if (enemies.length) await loadEnemy(enemies[0].id); else fillEnemy(defaultEnemy());
}
...
async function saveCurrent() {
  ...
  try {
    if (active === "leveling") {
      ...
      await request("/api/leveling", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ leveling }) });
    } else {
      const enemy = currentEnemy();
      const errors = enemy ? schema.validateEnemy(enemy) : ["Advanced JSON is invalid."];
      if (errors.length) throw new Error(errors.join("\n"));
      await request("/api/enemy", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enemy }) });
      currentId = enemy.id;
      const list = await request("/api/enemies");
      enemies = list.enemies;
      fillEnemy(enemy);
    }
    $("checks").innerHTML = '<div class="check ok">Saved successfully.</div>';
  } catch (error) {
...
function switchWorkspace(name) {
  ...
  if (name === "json") syncJson();
}

fields.forEach(id => $(id).addEventListener("input", () => { jsonEditing = false; sync(); }));
$("shape").addEventListener("change", sync);
$("level").addEventListener("input", drawPreview);
$("search").addEventListener("input", renderList);
$("newEnemy").addEventListener("click", () => fillEnemy(defaultEnemy()));
...
loadAll().catch(error => { $("checks").innerHTML = `<div class="check">${escapeHtml(error.message)}</div>`; });
