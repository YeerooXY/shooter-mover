...
(() => {
 ...
 document.addEventListener("keydown",e=>{if(["INPUT","TEXTAREA","SELECT"].includes(document.activeElement?.tagName))return;const k=e.key.toLowerCase();if((e.ctrlKey||e.metaKey)&&k==="c"){if(copy())e.preventDefault();return}if((e.ctrlKey||e.metaKey)&&k==="v"){e.preventDefault();paste();return}if((e.ctrlKey||e.metaKey)&&k==="d"){e.preventDefault();duplicate();return}if(e.ctrlKey||e.metaKey||e.altKey)return;const d={arrowleft:[-1,0],arrowright:[1,0],arrowup:[0,1],arrowdown:[0,-1]}[k];if(d&&nudge(d[0],d[1],e.shiftKey))e.preventDefault()},true);
})();
