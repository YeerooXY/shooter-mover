...
(() => {
  ...
  if (!presentation || !analysisButton || !playerLevelInput || !tierInput || !seedInput) return;
  ...
  window.fetch = async function captureStrongboxAnalysis(input, init) {
    const response = await nativeFetch(input, init);
    try {
      const url = typeof input === "string" ? input : input?.url || "";
      const request = init?.body ? JSON.parse(init.body) : null;
      if (url.includes("/api/strongbox-preview") && request?.mode === "analysis") {
        const payload = await response.clone().json();
        if (response.ok && payload?.ok) lastReport = payload;
      }
    } catch (_) {
      // Coverage is diagnostic-only and must never interfere with analysis.
    }
    return response;
  };
  ...
  function rarityKey(value) {
    ...
    for (const key of [...order].reverse()) if (text.includes(key)) return key;
    return "common";
  }

  async function postSingleProbe() {
    const response = await nativeFetch("/api/strongbox-preview", {
      method: "POST",
  ...
  async function ensureDefinitions() {
    const signature = `${playerLevelInput.value}:${tierInput.value}:${seedInput.value}`;
    if (definitions.length && signature === lastSignature) return definitions;
    if (loading) return loading;
    loading = postSingleProbe().then(probe => {
      const seen = new Set();
      definitions = (probe.candidates || []).filter(candidate => candidate.reason !== "not-live").filter(candidate => {
        ...
        rarityId: String(candidate.rarityId || ""),
  ...
  function coverageHtml(report) {
    if (!definitions.length) return `<section class="coverage-panel"><h3>Catalogue level coverage</h3><p>No live definitions were returned by the catalogue probe.</p></section>`;
    const player = Number(report.playerLevel || playerLevelInput.value || 1);
    const targetMinimum = Math.max(1, player + Number(report.minimumTargetDelta || 0));
    const targetLikely = Math.max(1, player + Number(report.mostLikelyTargetDelta || 0));
    const targetMaximum = Math.max(targetMinimum, player + Number(report.maximumTargetDelta || 0));
    ...
    const stats = rarities.map(key => {
      ...
      const near = values.filter(value => value.peakLevel >= targetMinimum && value.peakLevel <= targetMaximum).length;
    ...
    return `<section class="coverage-panel"><div class="coverage-head"><div><h3>Catalogue level coverage</h3></div><p>${definitions.length} live definitions · player ${player} · target ${targetMinimum}–${targetMaximum} · likely ${targetLikely}</p></div><div class="coverage-scroll"><div class="coverage-chart"><div class="coverage-axis"><div class="coverage-axis-label">Peak level</div><div class="coverage-track">${ticks.map(level => `<span class="coverage-tick" style="left:${percent(level, maximum)}%">${level}</span>`).join("")}</div></div>${rows}</div></div><div class="coverage-summary">${stats}</div><p class="coverage-note">Dots are authored weapon peak levels. White line: player level. Blue line: most likely box target. Shaded band: full target range. Click a dot to open that weapon in the Weapons tab when it appeared in the simulation.</p></section>`;
  }
  ...
  presentation.addEventListener("click", event => {
    const marker = event.target instanceof Element ? event.target.closest("[data-coverage-definition]") : null;
    if (!marker) return;
    const definitionId = marker.dataset.coverageDefinition;
    presentation.querySelector('[data-analysis-tab="weapons"]')?.click();
    requestAnimationFrame(() => {
      const weaponButton = [...presentation.querySelectorAll("[data-analysis-weapon]")].find(button => button.dataset.analysisWeapon === definitionId);
      weaponButton?.click();
    });
  });

  new MutationObserver(injectCoverage).observe(presentation, { childList: true, subtree: true, attributes: true, attributeFilter: ["class"] });
  [playerLevelInput, tierInput, seedInput].forEach(input => input.addEventListener("change", () => { definitions = []; lastSignature = ""; }));
  injectCoverage();
})();
