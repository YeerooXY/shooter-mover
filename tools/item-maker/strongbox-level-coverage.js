...
(() => {
  ...
  let catalogueProbePromise = null;
  ...
  function displayedReportContext() {
    ...
    const metrics = [...presentation.querySelectorAll(".analysis-metric")];
    const tierMetric = metrics.find(metric =>
    ...
    const targetMetric = metrics.find(metric => {
      ...
      return label === "Target level" || label === "Loot target";
    });

    const tierAndPlayer = tierMetric?.querySelector("strong")?.textContent?.split("/") || [];
    const tierNumber = parseInteger(tierAndPlayer[0]);
    const playerLevel = parseInteger(tierAndPlayer[1]);
    const targetRange = targetMetric?.querySelector("small")?.textContent || "";
    const rangeMatch = targetRange.match(/(-?\d+)\s*[–-]\s*(-?\d+)/);
    const minimumTargetLevel = rangeMatch ? Number(rangeMatch[1]) : NaN;
    const maximumTargetLevel = rangeMatch ? Number(rangeMatch[2]) : NaN;

    if (!seed
      || !Number.isInteger(tierNumber)
      || !Number.isInteger(playerLevel)
      || !Number.isInteger(minimumTargetLevel)
      || !Number.isInteger(maximumTargetLevel)) {
      return null;
    }
    ...
    return {
      ...
      minimumTargetLevel,
      maximumTargetLevel,
      centerTargetLevel: (minimumTargetLevel + maximumTargetLevel) / 2,
      signature: [
        seed,
        tierNumber,
        playerLevel,
  ...
  function loadCatalogueProbe() {
    if (catalogueProbePromise) return catalogueProbePromise;
    catalogueProbePromise = fetch("/api/strongbox-preview", {
      method: "POST",
  ...
  function coverageHtml(probe, context) {
    const definitions = definitionsFrom(probe);
    if (!definitions.length) {
      return `<section class="coverage-panel" data-coverage-signature="${escapeText(context.signature)}"><h3>Catalogue level coverage</h3><p>No live definitions were returned by the catalogue probe.</p></section>`;
    }

    const targetMinimum = Math.max(1, context.minimumTargetLevel);
    const targetMaximum = Math.max(targetMinimum, context.maximumTargetLevel);
    const targetCenter = Math.max(targetMinimum, Math.min(targetMaximum, context.centerTargetLevel));
    const maximum = Math.max(110, context.playerLevel, targetMaximum, ...definitions.map(value => value.peakLevel));
    ...
    const rows = rarities.map(key => {
      ...
      const markers = [...grouped.entries()]
        ...
        .join("");
      return `<div class="coverage-row"><div class="coverage-rarity" style="color:${view.glow}">${view.label} <small>(${values.length})</small></div><div class="coverage-track"><span class="coverage-window" style="left:${percent(targetMinimum, maximum)}%;width:${Math.max(.4, percent(targetMaximum, maximum) - percent(targetMinimum, maximum))}%" title="Observed target range ${targetMinimum}–${targetMaximum}"></span><span class="coverage-player" style="left:${percent(context.playerLevel, maximum)}%" title="Player level ${context.playerLevel}"></span><span class="coverage-center" style="left:${percent(targetCenter, maximum)}%" title="Observed target-range midpoint ${targetCenter}"></span>${markers}</div></div>`;
    }).join("");
    ...
    const stats = rarities.map(key => {
      ...
      return `<div class="coverage-stat"><span style="color:${view.color}">${view.label}</span><strong>${near} in observed band</strong><small>largest gap ${gap.size} levels${gap.size ? ` · ${gap.from}→${gap.to}` : ""}</small></div>`;
    }).join("");

    return `<section class="coverage-panel" data-coverage-signature="${escapeText(context.signature)}"><div class="coverage-head"><div><h3>Catalogue level coverage</h3></div><p>${definitions.length} live definitions · player ${context.playerLevel} · observed targets ${targetMinimum}–${targetMaximum}</p></div><div class="coverage-scroll"><div class="coverage-chart"><div class="coverage-axis"><div class="coverage-axis-label">Peak level</div><div class="coverage-track">${ticks.map(level => `<span class="coverage-tick" style="left:${percent(level, maximum)}%">${level}</span>`).join("")}</div></div>${rows}</div></div><div class="coverage-summary">${stats}</div><p class="coverage-note">Dots are authored weapon peak levels. White line: player level. Blue line: midpoint of the observed target range. Shaded band: minimum and maximum target levels seen in this analysis. Click a dot to open a matching weapon in the Weapons tab when one appeared in the simulation.</p></section>`;
  }
  ...
  function injectCoverage() {
    ...
    loadCatalogueProbe().then(probe => {
      if (revision !== renderRevision) return;
      const currentContent = presentation.querySelector("#analysisTabContent");
      const currentContext = displayedReportContext();
      const levelsStillActive = presentation.querySelector('.analysis-tab.active[data-analysis-tab="levels"]');
      const placeholder = currentContent?.querySelector(".coverage-loading");
      if (!currentContent || !levelsStillActive || !currentContext) return;
      if (currentContext.signature !== context.signature) return;
      if (!placeholder || placeholder.dataset.coverageSignature !== context.signature) return;
      placeholder.outerHTML = coverageHtml(probe, context);
    }).catch(error => {
      if (revision !== renderRevision) return;
      const currentContent = presentation.querySelector("#analysisTabContent");
      const currentContext = displayedReportContext();
