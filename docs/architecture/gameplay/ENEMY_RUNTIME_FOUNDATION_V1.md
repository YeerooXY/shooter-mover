...
`EnemyLiveView` maps the actor's `ActorId` to ENTITY-001
...
Room completion consumes `EnemyLiveView.BlocksRoomClear`. Active required enemies and
...
| Mobile Blaster Droid | `MobileBlasterDroidDefinition`, including shared projection factory | `MobileBlasterDroidRuntime2D` over `EnemyActorState` | Approach/retreat velocity in runtime | Mobile blaster projectile, projected as its existing attack reference | Configured `EnemyTarget2DBridge` | Required enemy (implemented projection) | External XP/drop consumers | Runtime2D and temporary presentation | Move live decision calculation to supplied perception and execute generic intent |
