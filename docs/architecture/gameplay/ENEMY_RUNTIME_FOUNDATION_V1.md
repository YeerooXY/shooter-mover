...
| Mobile Blaster Droid | `MobileBlasterDroidDefinition` | `MobileBlasterDroidRuntime2D` over `EnemyActorState` | Approach/retreat velocity in runtime | Mobile blaster projectile | Configured `EnemyTarget2DAdapter` | Required enemy (expected projection) | External XP/drop consumers | Runtime2D and temporary presentation | Move decision calculation to supplied perception and execute generic intent |
