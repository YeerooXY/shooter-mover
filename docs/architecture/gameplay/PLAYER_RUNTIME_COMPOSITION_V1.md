...
The composition reuses `PlayerActorState` as the only health/death authority. It adds explicit adapter boundaries around the already-existing movement lifecycle, presentation, input ownership, trusted participant attribution, HUD projection, and run death facts.

## Ownership boundaries

| Capability | Owner | This component may do | This component must not do |
| --- | --- | --- | --- |
| Player identity, lifecycle generation, health, death, damage/healing replay | `PlayerActorState` | Construct once, forward commands, read immutable snapshots | Create fallback health, mutate health directly, award rewards |
| Position, velocity, movement generation, thruster state | Existing `MovementActor2D` through `MovementActorPlayerLiveBridge` | Read immutable movement snapshots, request explicit restart, dispose the owned lifecycle adapter | Duplicate locomotion or physics authority |
| Sprite, mounts, boost trail, visual reset | `IPlayerPresentationLive` implementation | Refresh continuous boost and project restart snapshots | Decide health, death, movement, attribution, or rewards |
| Local/future network input | `IPlayerInputLive` implementation | Acquire one explicit actor/participant ownership lease and release it exactly once | Become a global input singleton or infer participant ownership from a client claim |
...
| Run statistics/death attribution | `IPlayerRunFlow` implementation | Observe immutable `GameplayEntityDeathFact` values | Award kills, XP, money, inventory, drops, or routing |

## Construction and failure policy

`PlayerLiveSetupRoot` permits one successful runtime construction and contains no static registry. Failed validation does not create fallback truth or acquire input ownership. Construction requires all of the following:

- an immutable `PlayerActorDefinition` inside `PlayerLiveConfiguration`;
...
Missing configuration fails closed. No default actor identity, participant, character, faction, health, or generation is invented. The movement generation must exactly match the initial `PlayerActorState` lifecycle generation before input ownership is acquired.
...
The adapter resolves trusted participant attribution from `ITrustedPlayerAttributionResolver` using the authoritative source actor identity. The resulting `DamageReceiverCommand` is processed by `PlayerActorState`, which retains damage replay protection, generation validation, target validation, health mutation, and death-fact production. Healing uses the same resolver before creating `PlayerActorHealingCommand`; untrusted participant claims are never forwarded for either path.
...
The composition validates the entire command before mutation. It then restarts movement, restarts `PlayerActorState` with the same operation ID and generations, exports the combined snapshot, and asks presentation to reset from that snapshot. Exact restart replay is returned as a duplicate without restarting either capability again. Stale or future generations are rejected before either authority moves.

`MovementActorPlayerLiveBridge` delegates to the existing `MovementActorLifecycle.RestartActor()` and verifies that the movement actor reached the requested generation.
...
1. **Construct runtime**
   ...
   - Wrap `MovementActorLifecycle` with `MovementActorPlayerLiveBridge`.
   ...
   - Construct one `PlayerLiveSetupRoot`.
...
4. **Delegate restart, boost, and disposal**
   - Replace local restart ordering with one `PlayerLiveRestartCommand`.
